 /*
 * Author - Mike Blandford
 *
 * Based on er9x by Erez Raviv <erezraviv@gmail.com>
 *
 * Based on th9x -> http://code.google.com/p/th9x/
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 */

// This version for ARM based ERSKY9X board

#include <stdint.h>
#include <stdlib.h>
#include <string.h>

#if defined(PCBX12D) || defined(PCBX10)
//#define	WHERE_TRACK		1
//void notePosition( uint8_t byte ) ;
#endif

#ifdef PCBSKY
 #ifndef PCBDUE
 #include "AT91SAM3S4.h"
 #endif

//#define RESET_DISPLAY	1

#endif
#include "ersky9x.h"
#include "myeeprom.h"
#include "lcd.h"
#include "drivers.h"
#include "logicio.h"
#ifdef PCB9XT
#include "mega64.h"
#endif

#ifndef PCBX12D
#ifndef PCBX10
 #ifndef PROP_TEXT
#include "lbm/font.lbm"
#define font_5x8_x20_x7f (font)
#include "lbm/font_dblsize.lbm"
#define font_10x16_x20_x7f (font_dblsize)

#include "font12x8test.lbm"
 #endif

#ifndef PROP_TEXT
#ifndef SMALL
 #ifndef PCBLEM1
const uint8_t font_se_extra[] = {
#include "lbm/font_se_05x07.lbm"
} ;
 #endif
#endif
const uint8_t font_fr_extra[] = {
#include "lbm/font_fr_05x07.lbm"
} ;
const uint8_t font_de_extra[] = {
#include "lbm/font_de_05x07.lbm"
} ;
#ifndef SMALL
 #ifndef PCBLEM1
const uint8_t font_it_extra[] = {
#include "lbm/font_it_05x07.lbm"
} ;
const uint8_t font_pl_extra[] = {
#include "lbm/font_pl_05x07.lbm"
} ;
 #endif
#endif
#endif
#endif

 #ifndef PROP_TEXT
#ifndef SMALL
 #ifndef PCBLEM1
const uint8_t font_se_big_extra[] = {
#include "lbm/font_se_10x14.lbm"
} ;
 #endif
#endif
const uint8_t font_fr_big_extra[] = {
#include "lbm/font_fr_10x14.lbm"
} ;
const uint8_t font_de_big_extra[] = {
#include "lbm/font_de_10x14.lbm"
} ;
#endif
 #endif // PROP_TEXT


 #ifndef PROP_TEXT
#include "lbm/font_04x06.lbm"
 #endif // PROP_TEXT
//#define font_5x8_x20_x7f (font)

 #ifndef PROP_TEXT
const uint8_t *ExtraFont = NULL ;
const uint8_t *ExtraBigFont = NULL ;
 #endif // PROP_TEXT

// Local data
//#ifdef INVERT_DISPLAY
//extern uint16_t LcdBits ;
//extern uint16_t LcdErrors ;
//#endif
uint8_t Lcd_lastPos ;
uint8_t plotType = PLOT_XOR ;

uint8_t plotStack[4] ;
uint8_t plotStackIndex ;

void pushPlotType( uint8_t type )
{
	if ( plotStackIndex < 4 )
	{
		plotStack[plotStackIndex++] = plotType ;
	}
	plotType = type ;
}

void popPlotType()
{
	if ( plotStackIndex )
	{
		plotStackIndex -= 1 ;
	}
	plotType = plotStack[plotStackIndex] ;
}


#ifndef PCBX12D
#ifndef PCBX10
#ifdef GREY_SCALE
uint8_t DisplayBuf[DISPLAY_W*DISPLAY_H/8*4] ;
#else
uint8_t DisplayBuf[DISPLAY_W*DISPLAY_H/8] ;
#endif
#define DISPLAY_END (DisplayBuf+sizeof(DisplayBuf))
#endif
#endif

#if defined(PCBX12D) || defined(PCBX10)
extern uint32_t CurrentFrameBuffer ;

#define SDRAM_BANK_ADDR     ((uint32_t)0xD0000000)
//#define LCD_W	480
//#define LCD_H	272
#endif

#if defined(PCBX12D) || defined(PCBX10)
uint16_t LcdBackground = LCD_BACKGROUND ;
uint16_t LcdForeground = LCD_BLACK ;
uint16_t LcdCustomColour = LCD_BLACK ;
uint16_t DimBackColour ;
#endif

#if defined(PCBX7) || defined (PCBXLITE) || defined (PCBX9LITE) || defined(REV19)
#define X9D_OFFSET		0
#define DISPLAY_START (DisplayBuf + 0)
#else // PCBX7/LITE/X3
#ifdef PCBX9D
#define X9D_OFFSET		11
#define DISPLAY_START (DisplayBuf + X9D_OFFSET)
#else
#define DISPLAY_START (DisplayBuf + 0)
#endif 
#endif // PCBX7

#if defined(PCBSKY) || defined(PCB9XT) || defined(PCBX7) || defined (PCBXLITE) || defined(PCBX9LITE)
uint8_t ExtDisplayBuf[DISPLAY_W*DISPLAY_H/8 + 2] ;
uint16_t ExtDisplayTime ;
uint8_t ExtDisplaySend ;
#endif 

#ifndef PCBDUE
#ifdef PCBSKY
// Lookup table for prototype board
#ifdef REVA
const uint8_t Lcd_lookup[] =
{

} ;
#endif 
#endif 
#endif // DUE

//#if defined(PCBX12D) || defined(PCBX10)
//uint16_t DisplayOffset ;
//#endif

#if defined(PCBX12D) || defined(PCBX10)
void putsTimexxl( uint16_t x, uint16_t y, int16_t tme, uint8_t att, uint16_t colour )
{
	div_t qr ;
	uint32_t i ;
//	uint32_t j ;
	// Can't handle negative (yet)
	char text[8] ;
	char *p = text ;

	if ( tme < 0 )
	{
		tme = -tme ;
		*p++ = '-' ;
		x -= getTextWidth( text, 1, XXLSIZE ) ;
	}
//	lcdDrawCharxxlTransparent( x+84, y, ':', att, colour ) ;

	qr = div( tme, 60 ) ;
	i = qr.quot / 10 ;
	*p++ = '0'+ i ;
//	lcdDrawCharxxlTransparent( x+24, y, i + '0', att, colour ) ;
	i = qr.quot % 10 ;
	*p++ = '0'+ i ;
//	lcdDrawCharxxlTransparent( x+60, y, i + '0', att, colour ) ;
	*p++ = ':' ;
	i = qr.rem / 10 ;
	*p++ = '0'+ i ;
//	lcdDrawCharxxlTransparent( x+112, y, i + '0', att, colour ) ;
	i = qr.rem % 10 ;
	*p++ = '0'+ i ;
//	lcdDrawCharxxlTransparent( x+148, y, i + '0', att, colour ) ;
	*p = 0 ;
	i = p - text ;
//	i = getTextWidth( text, j, XXLSIZE ) ;
	lcdDrawSizedText( x, y, text, i, XXLSIZE, colour ) ;

}
#endif

#if defined(PCBX12D) || defined(PCBX10)
void putsTime(uint16_t x,uint16_t y,int16_t tme,LcdFlags att,LcdFlags att2, uint16_t colour, uint16_t background)
#else
void putsTime(coord_t x,coord_t y,int16_t tme,LcdFlags att,LcdFlags att2)
#endif
{
	div_t qr ;

#if defined(PCBX12D) || defined(PCBX10)
	x *= HVSCALE ;
	y *= HVSCALE ;
	uint32_t neg = 0 ;
	if ( tme<0 )
	{
		neg = 1 ;
		tme = -tme;
	}
	qr = div( tme, 60 ) ;
	lcdDrawChar( x, y, ':', att ) ;
	lcdDrawNumber( LcdNextPos, y, qr.rem, att2|LEADING0|LEFT, 2 ) ;
	tme = neg ? -qr.quot : qr.quot ;
	lcdDrawNumber( x, y, tme, att|LEADING0, 2 ) ;

#else
	uint8_t z = FWNUM*6-2 ;
	if ( att&DBLSIZE )
	{
		if ( att&CONDENSED )
		{
			x += 3 ;
			z = FWNUM*5-2 ;
		}
	}
	if ( tme<0 )
	{
 #if defined(PCBX12D) || defined(PCBX10)
		lcdDrawChar( x - ((att&DBLSIZE) ? z : FWNUM*3),    y, '-',att, colour, background ) ;
 #else
		PUTC_ATT( x - ((att&DBLSIZE) ? z : FWNUM*3),    y, '-',att);
 #endif
		tme = -tme;
	}

 #if defined(PCBX12D) || defined(PCBX10)
	lcd_putcAttColour( x, y, ':',att&att2, colour, background ) ;
 #else
 	PUTC_ATT( x, y, ':',att&att2);
 #endif
	qr = div( tme, 60 ) ;
	if ( att&DBLSIZE )
	{
		if ( att&CONDENSED )
		{
 #if defined(PCBX12D) || defined(PCBX10) || defined(PROP_TEXT)
 #else
			x += 2 ;
 #endif
		}
	}
 #if defined(PCBX12D) || defined(PCBX10)
	lcd_2_digits( x, y, (uint16_t)qr.quot, att, colour, background ) ;
 #else
//	lcd_2_digits( x, y, (uint16_t)qr.quot, att ) ;
	PUTS_NUM_N( x, y, (uint16_t)qr.quot, att + LEADING0, 2 ) ;
 #endif
	if ( att&DBLSIZE )
	{
		if ( att&CONDENSED )
		{
 #if defined(PCBX12D) || defined(PCBX10) || defined(PROP_TEXT)
			x += FWNUM*4-3 ;
 #else
			x += FWNUM*4+2 ;
 #endif
		}
		else
		{
			x += FWNUM*6-4 ;
		}
	}
	else
	{
		x += FW*3-4 ;
	}
 #if defined(PCBX12D) || defined(PCBX10)
	lcd_2_digits( x, y, (uint16_t)qr.rem, att2 + LEADING0, colour, background ) ;
 #else
//	lcd_2_digits( x, y, (uint16_t)qr.rem, att2 ) ;
	PUTS_NUM_N( x, y, (uint16_t)qr.rem, att + LEADING0, 2 ) ;
 #endif

#endif

}

#if defined(PCBX12D) || defined(PCBX10)
void putsVolts(uint16_t x,uint16_t y, uint8_t volts, LcdFlags att)
{
	uint8_t option = att & NO_UNIT ;
	att &= ~NO_UNIT ;
	lcdDrawNumber(x*HVSCALE, y*HVSCALE, volts, att|PREC1) ;
	if(!(option&NO_UNIT)) lcdDrawChar(LcdLastRightPos, y*HVSCALE, 'V', 0 ) ;
}

void putsVBat(uint16_t x,uint16_t y,LcdFlags att)
{
#ifndef MINIMISE_CODE    
  att |= g_vbat100mV < g_eeGeneral.vBatWarn ? BLINK : 0;
#endif
	putsVolts(x, y, g_vbat100mV, att);
}


#else
void putsVolts(coord_t x,coord_t y, uint8_t volts, LcdFlags att)
{
	uint8_t option = att & NO_UNIT ;
	att &= ~NO_UNIT ;
#if defined(PROP_TEXT)
	lcdDrawNumber(x, y, volts, att|PREC1);
	if(!(option&NO_UNIT)) lcdDrawChar(LcdLastRightPos, y, 'V', 0);
#else
	PUTS_NUM(x, y, volts, att|PREC1);
	if(!(option&NO_UNIT)) PUTC_ATT(Lcd_lastPos, y, 'V', 0);
#endif
}

void putsVBat(coord_t x,coord_t y,LcdFlags att)
{
#ifndef MINIMISE_CODE    
  att |= g_vbat100mV < g_eeGeneral.vBatWarn ? BLINK : 0;
#endif
	putsVolts(x, y, g_vbat100mV, att);
}

#endif


#if defined(PCBX12D) || defined(PCBX10)

extern uint16_t Mimage[] ;
extern uint16_t *SdramImage ;
void lcd_picture( uint16_t i_x, uint16_t i_y, uint16_t maxHeight, uint16_t maxWidth )
{
//	uint16_t *p ;
//	uint32_t rows ;
	uint16_t *src = SdramImage ;
	uint16_t height ;
	uint16_t reduceWidth = 0 ;

extern uint16_t Image_width ;
extern uint16_t Image_height ;

	height = Image_height ;
	if ( height > maxHeight )
	{
		uint16_t t ;
		t = (height-maxHeight) / 2 ;
		height = maxHeight ;
		src += Image_width * t ;
//		i_y += t ;
	}
	if ( Image_width > maxWidth )
	{
		reduceWidth = Image_width - maxWidth ;
	}

	DMAcopyImage( i_x, i_y, Image_width, height, src, reduceWidth ) ;

//	for ( rows = 0 ; rows < Image_height ; rows += 1 )
//	{
//		p = ( uint16_t *) CurrentFrameBuffer ;
//		p += i_x ;
//		p += i_y * LCD_W ;
//    memmove( p, src, Image_width*2 ) ;
//		src += Image_width ;
//		i_y += 1 ;	 
//	} 
}

void lcd_HiResbitmap( uint16_t i_x, uint16_t i_y, PROGMEM *bitmap, uint8_t w, uint8_t h, LcdFlags mode, uint16_t colour, uint16_t background )
{
	uint32_t yb ;
	uint32_t x ;
	uint16_t *p ;
	uint8_t mask ;
  uint32_t hb   = (h + 7) / 8 ;
  uint32_t hlast ;

#ifdef INVERT_DISPLAY
	i_x = (LCD_W-1) - i_x ; // - (w - 1) ;
	i_y = (LCD_H-1) - i_y ; // - (h - 1) ;
	if ( i_y >= LCD_H )
	{
		i_y = 0 ;
	}
#endif
	 
  register bool inv = false ;
	if (mode & INVERS) inv = true ;
	if ( (mode & BLINK) && BLINK_ON_PHASE )
	{
		inv = !inv ;
	}
//	bool inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false ) ;
  for( yb = 0; yb < hb; yb++)
	{
		p = ( uint16_t *) CurrentFrameBuffer ;
		p += i_x ;
		p += i_y * LCD_W ;
#ifdef INVERT_DISPLAY
		p -= yb * LCD_W * 8 ;
#else
		p += yb * LCD_W * 8 ;
#endif
    for(x=0; x < w; x++)
		{
      register uint8_t b = *bitmap++ ;
			if ( inv )
			{
				b = ~b ;
			}
			uint16_t *q = p ;

			hlast = ( h < 8 ) ? h : 9 ;
			for ( mask = 1 ; mask ; mask <<= 1 )
			{
				uint16_t value = (b & mask) ? colour : background ;
				*q = value ;
#ifdef INVERT_DISPLAY
				q -= LCD_W ;
#else
				q += LCD_W ;
#endif
				if ( --hlast == 0 )
				{
					break ;
				}
			}
#ifdef INVERT_DISPLAY
			p -= 1 ;
#else
			p += 1 ;
#endif
    }
		h -= 8 ;
  }
	
}

void lcd_HiResimg( uint16_t i_x, uint16_t i_y, PROGMEM *imgdat, uint8_t idx, LcdFlags mode, uint16_t colour, uint16_t background )
{
  register const unsigned char *q = imgdat ;
  register uint8_t w    = *q++ ;
  register uint32_t hb   = *q++ ;
  register uint8_t sze1 = *q++ ;
//	uint32_t yb ;
//	uint32_t x ;

  q += idx * sze1 ;
  
	lcd_HiResbitmap( i_x, i_y, q, w, hb, mode, colour, background ) ;
}



void lcd_bitmap( uint8_t i_x, uint8_t i_y, PROGMEM *bitmap, uint8_t w, uint8_t h, LcdFlags mode, uint16_t colour, uint16_t background )
{
	uint32_t yb ;
	uint32_t x ;
	uint16_t *p ;
	uint8_t mask ;
  uint32_t hb   = (h + 7) / 8 ;
  uint32_t hlast ;

#ifdef INVERT_DISPLAY
	i_x = (LCD_W/2-1) - i_x ; // - (w - 1) ;
	i_y = (LCD_H/2-1) - i_y ; // - (h - 1) ;
	if ( i_y >= LCD_H/2 )
	{
//		LcdBits |= 16 ;
//		LcdErrors += 1 ;
		i_y = 0 ;
	}
//	i_x = (LCD_W/2-1) - i_x ;
//	i_y = (LCD_H/2-1) - i_y ;
#endif
	 
  register bool inv = false ;
	if (mode & INVERS) inv = true ;
	if ( (mode & BLINK) && BLINK_ON_PHASE )
	{
		inv = !inv ;
	}
//	bool inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false ) ;
  for( yb = 0; yb < hb; yb++)
	{
		p = ( uint16_t *) CurrentFrameBuffer ;
		p += i_x*2 ;
		p += i_y * LCD_W * 2 ;
#ifdef INVERT_DISPLAY
		p -= yb * LCD_W * 2 * 8 ;
#else
		p += yb * LCD_W * 2 * 8 ;
#endif
    for(x=0; x < w; x++)
		{
      register uint8_t b = *bitmap++ ;
			if ( inv )
			{
				b = ~b ;
			}
			uint16_t *q = p ;

			hlast = ( h < 8 ) ? h : 9 ;
			for ( mask = 1 ; mask ; mask <<= 1 )
			{
				uint16_t value = (b & mask) ? colour : background ;
				*q = value ;
				*(q+1) = value ;
				*(q+LCD_W) = value ;
				*(q+LCD_W+1) = value ;
#ifdef INVERT_DISPLAY
				q -= LCD_W * 2 ;
#else
				q += LCD_W * 2 ;
#endif
				if ( --hlast == 0 )
				{
					break ;
				}
			}
#ifdef INVERT_DISPLAY
			p -= 2 ;
#else
			p += 2 ;
#endif
    }
		h -= 8 ;
  }
}

void lcd_img( uint8_t i_x, uint8_t i_y, PROGMEM *imgdat, uint8_t idx, LcdFlags mode, uint16_t colour, uint16_t background )
{
  register const unsigned char *q = imgdat ;
  register uint8_t w    = *q++ ;
  register uint32_t hb   = *q++ ;
  register uint8_t sze1 = *q++ ;
//	uint32_t yb ;
//	uint32_t x ;

  q += idx * sze1 ;
  
	lcd_bitmap( i_x, i_y, q, w, hb, mode, colour, background ) ;
}

void lcdSetRefVolt( uint8_t val )
{
	
}

#else
void lcd_bitmap( coord_t i_x, coord_t i_y, PROGMEM *bitmap, uint8_t w, uint8_t h, LcdFlags mode )
{
	uint32_t yb ;
	uint32_t x ;
	
  register bool inv = false ;
	if (mode & INVERS) inv = true ;
	if ( (mode & BLINK) && BLINK_ON_PHASE )
	{
		inv = !inv ;
	}
//	bool inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false ) ;
  for( yb = 0; yb < h; yb++)
	{
    register uint8_t *p = &DISPLAY_START[ (i_y / 8 + yb) * DISPLAY_W + i_x ];
#if PCBX9D
		if ( i_x > 211-X9D_OFFSET )
		{
			p -= DISPLAY_W ;		
		}
#endif
    for(x=0; x < w; x++)
		{
      register uint8_t b = *bitmap++ ;
      *p++ = inv ? ~b : b;
    }
  }
	
}

void lcd_img( coord_t i_x, coord_t i_y, PROGMEM *imgdat, uint8_t idx, LcdFlags mode )
{
  register const unsigned char *q = imgdat ;
  register uint8_t w    = *q++ ;
  register uint32_t hb   = (*q++ +7) / 8 ;
  register uint8_t sze1 = *q++ ;
//	uint32_t yb ;
//	uint32_t x ;

  q += idx * sze1 ;
  
	lcd_bitmap( i_x, i_y, q, w, hb, mode ) ;
}
#endif

#if defined(PCBX12D) || defined(PCBX10) || defined(PROP_TEXT)
#else
uint8_t lcd_putc(coord_t x,coord_t y,const char c )
{
  return lcd_putcAtt(x,y,c,0);
}
#endif

#if defined(PCBX12D) || defined(PCBX10)
//uint16_t lcd_putcAttDblColour(uint16_t x,uint16_t y,const char c,LcdFlags mode, uint16_t colour, uint16_t background )
//{
//	if ( c < 31 )
//	{
//		x = c*FW ;
//		if ( (mode & CONDENSED) )
//		{
//			x = c*8 ;
//		}
//		else
//		{
//			x += x ;
//		}	 
//		return x ;
//	}
//	lcdDrawCharBitmapDoubleDma( x*2, y*2, c, mode, colour ) ;
//	if (mode & CONDENSED)
//	{
//		x += 8 ;
//	}
//	else
//	{
//		x += FW*2 ;
//	}
//	return x ;


////	uint16_t *p ;
////	uint16_t *r ;
////  uint8_t *q ;
////	uint8_t mask ;
////	int32_t i ;

////	r = ( uint16_t *) CurrentFrameBuffer ;
////	r += x*2 ;
////	r += y * LCD_W * 2 ;
////	x += FW ;
////	uint32_t doNormal = 1 ;
////	unsigned char c_mapped = c ;
////  register bool inv = false ;
////	if (mode & INVERS) inv = true ;
////	if ( (mode & BLINK) && BLINK_ON_PHASE )
////	{
////		inv = !inv ;
////	}
//////  register bool   inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false);
	
////	if ( (mode & CONDENSED) )
////	{
////		doNormal = 0 ;
			
////		if ( doNormal == 0 )
////		{
////			if ( (c!=0x2E)) x+=8-FW; //check for decimal point
////		/* each letter consists of 8 top bytes followed by
////	 	* five bottom by 8 bottom bytes (16 bytes per 
////	 	* char) */
////			if( c < 0xC0 )
////			{
////				c_mapped = c - 0x20 ;
////				q = (uint8_t *) &font_12x8[c_mapped*14] ;
////			}
////			else
////			{
////				q = (uint8_t *) &font_12x8[0] ;
////			}
    		
////			for( i=7 ; i>=0 ; i-- )
////			{
////				uint8_t b1 ;
////				uint8_t b3 ;

////  		  /*top byte*/
////   		  b1 = *q ;
////  		  /*bottom byte*/
////   		  b3 = *(q+7) ;
////   		  q++;
////				if ( i == 0 )
////				{
////					b1 = 0 ;
////					b3 = 0 ;
////				}
////    		if(inv)
////				{
////				  b1=~b1;
////				  b3=~b3;
////    		}
////				p = r ;
////				for ( mask = 1 ; mask ; mask <<= 1 )
////				{
////					uint16_t v = ( b1 & mask ) ? colour : background ;
////					*p = v ;
////					*(p+1) = v ;
////					*(p+LCD_W) = v ;
////					*(p+LCD_W+1) = v ;
////					v = ( b3 & mask ) ? colour : background ;
////					*(p+LCD_W*16) = v ;
////					*(p+LCD_W*16+1) = v ;
////					*(p+LCD_W*17) = v ;
////					*(p+LCD_W*17+1) = v ;
////					p += LCD_W * 2 ;
////				}
////				r += 2 ;
////    	}
////		}
////	}

////	if ( doNormal )
////	{
////		if ( (c!=0x2E)) x+=FW; //check for decimal point
////	/* each letter consists of ten top bytes followed by
//// 	* five bottom by ten bottom bytes (20 bytes per 
//// 	* char) */
////		q = (uint8_t *) &font_dblsize[(c-0x20)*20] ;
////	  for( i=11 ; i>=0 ; i-- )
////		{
////			uint8_t b1 ;
////			uint8_t b2 ;

////		  /*top byte*/
////	    b1 = i>1 ? *q : 0;
////		  /*bottom byte*/
////	    b2 = i>1 ? *(q+10) : 0;
////			if ( inv )
////			{
////				b1 = ~b1 ;
////				b2 = ~b2 ;
////			}
////	    q++;
////			p = r ;
////			for ( mask = 1 ; mask ; mask <<= 1 )
////			{
////				uint16_t v = ( b1 & mask ) ? colour : background ;
////				*p = v ;
////				*(p+1) = v ;
////				*(p+LCD_W) = v ;
////				*(p+LCD_W+1) = v ;
////				v = ( b2 & mask ) ? colour : background ;
////				*(p+LCD_W*16) = v ;
////				*(p+LCD_W*16+1) = v ;
////				*(p+LCD_W*17) = v ;
////				*(p+LCD_W*17+1) = v ;
////				p += LCD_W * 2 ;
////			}
////			r += 2 ;
////		}
////	}
////	return x ;
//}

//uint16_t lcd_putcAttDbl(uint16_t x,uint16_t y,const char c,uint8_t mode)
//{
//	uint16_t *p ;
//	uint16_t *r ;
//  uint8_t *q ;
//	uint8_t mask ;
//	int32_t i ;

//	r = ( uint16_t *) CurrentFrameBuffer ;
//	r += x*2 ;
//	r += y * LCD_W * 2 ;
//	if ( c < 22 )
//	{
//		x = c*FW ;
//		if ( (mode & CONDENSED) )
//		{
//			x = c*8 ;
//		}
//			else
//		{
//			x += x ;
//		}	 
//		return x ;
//	}
//	x += FW ;
//	uint32_t doNormal = 1 ;
//	unsigned char c_mapped = c ;
  
//	register bool inv = false ;
//	if (mode & INVERS) inv = true ;
//	if ( (mode & BLINK) && BLINK_ON_PHASE )
//	{
//		inv = !inv ;
//	}
////  register bool   inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false);
	
//	if ( (mode & CONDENSED) )
//	{
//		doNormal = 0 ;
			
//		if ( doNormal == 0 )
//		{
//			if ( (c!=0x2E)) x+=8-FW; //check for decimal point
//		/* each letter consists of 8 top bytes followed by
//	 	* five bottom by 8 bottom bytes (16 bytes per 
//	 	* char) */
//			if( c < 0xC0 )
//			{
//				c_mapped = c - 0x20 ;
//				q = (uint8_t *) &font_12x8[c_mapped*14] ;
//			}
//			else
//			{
//				q = (uint8_t *) &font_12x8[0] ;
//			}
    		
//			for( i=7 ; i>=0 ; i-- )
//			{
//				uint8_t b1 ;
//				uint8_t b3 ;

//  		  /*top byte*/
//   		  b1 = *q ;
//  		  /*bottom byte*/
//   		  b3 = *(q+7) ;
//   		  q++;
//				if ( i == 0 )
//				{
//					b1 = 0 ;
//					b3 = 0 ;
//				}
//    		if(inv)
//				{
//				  b1=~b1;
//				  b3=~b3;
//    		}
//				p = r ;
//				for ( mask = 1 ; mask ; mask <<= 1 )
//				{
//					if ( b1 & mask )
//					{
//						*p ^= 0xFFFF ;
//						*(p+1) ^= 0xFFFF ;
//						*(p+LCD_W) ^= 0xFFFF ;
//						*(p+LCD_W+1) ^= 0xFFFF ;
//					}
//					if ( b3 & mask )
//					{
//						*(p+LCD_W*16) ^= 0xFFFF ;
//						*(p+LCD_W*16+1) ^= 0xFFFF ;
//						*(p+LCD_W*17) ^= 0xFFFF ;
//						*(p+LCD_W*17+1) ^= 0xFFFF ;
//					}
//					p += LCD_W * 2 ;
//				}
//				r += 2 ;
//    	}
//		}
//	}

//	if ( doNormal )
//	{
//		if ( (c!=0x2E)) x+=FW; //check for decimal point
//	/* each letter consists of ten top bytes followed by
// 	* five bottom by ten bottom bytes (20 bytes per 
// 	* char) */
//		q = (uint8_t *) &font_dblsize[(c-0x20)*20] ;
//	  for( i=11 ; i>=0 ; i-- )
//		{
//			uint8_t b1 ;
//			uint8_t b2 ;

//		  /*top byte*/
//	    b1 = i>1 ? *q : 0;
//		  /*bottom byte*/
//	    b2 = i>1 ? *(q+10) : 0;
//	    q++;
//			p = r ;
//			for ( mask = 1 ; mask ; mask <<= 1 )
//			{
//				if ( b1 & mask )
//				{
//					*p ^= 0xFFFF ;
//					*(p+1) ^= 0xFFFF ;
//					*(p+LCD_W) ^= 0xFFFF ;
//					*(p+LCD_W+1) ^= 0xFFFF ;
//				}
//				if ( b2 & mask )
//				{
//					*(p+LCD_W*16) ^= 0xFFFF ;
//					*(p+LCD_W*16+1) ^= 0xFFFF ;
//					*(p+LCD_W*17) ^= 0xFFFF ;
//					*(p+LCD_W*17+1) ^= 0xFFFF ;
//				}
//				p += LCD_W * 2 ;
//			}
//			r += 2 ;
//		}
//	}
//	return x ;
	
//}

#ifndef COLOUR_DISPLAY
uint16_t lcd_putcAttColour(uint16_t x,uint16_t y,const char c,LcdFlags mode, uint16_t colour, uint16_t background )
{
	if ( (mode & BLINK) && BLINK_ON_PHASE )
	{
		mode ^= INVERS ;
	}
	if ( mode & DBLSIZE )
	{
		return lcd_putcAttDblColour( x, y, c, mode, colour, background ) ;
	}
	if ( c < 22 )		// Move to specific x position (c)*FW
	{
		x = c*FW ;
  	if(mode&DBLSIZE)
		{
			if ( (mode & CONDENSED) )
			{
				x = c*8 ;
			}
		 	else
			{
				x += x ;
			}	 
		}
		return x ;
	}

	lcdDrawCharBitmapDma( x*2, y*2, c, mode, colour ) ;
	x += FW ;
	if (mode & CONDENSED)
	{
		x += FWNUM-FW ;
	}
	return x ;
	
//	uint16_t *p ;
//  uint8_t *q ;
//	uint8_t mask ;
//	uint16_t v ;
//	p = ( uint16_t *) CurrentFrameBuffer ;
//	p += x*2 ;
//	p += y * LCD_W * 2 ;
	 
//	q = (uint8_t *) &font_5x8_x20_x7f[(c-0x20)*5] ;
	
//  register bool inv = false ;
//	if (mode & INVERS) inv = true ;
//	if ( (mode & BLINK) && BLINK_ON_PHASE )
//	{
//		inv = !inv ;
//	}
////  register bool   inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false);

//	uint8_t condense = 0 ;

//	if (mode & CONDENSED)
//	{
////		*p = inv ? ~0 : 0;
//		p += 2 ;
//		condense = 1 ;
//		x += FWNUM-FW ;
//	}
	 
//	for ( mask = 1 ; mask ; mask <<= 1 )
//	{
//		uint16_t *r = p ;
//		uint8_t b ;

//		b = *q ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? colour : background ;
//		*r = v ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		if ( condense == 0 )
//		{
//			b = *(q+1) ;
//			if ( inv ) b = ~b ;
//			v = ( b & mask ) ? colour : background ;
//			*r = v ;
//			*(r+1) = v ;
//			*(r+LCD_W) = v ;
//			*(r+LCD_W+1) = v ;
//			r += 2 ;
//		}
//		b = *(q+2) ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? colour : background ;
//		*r = v ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		b = *(q+3) ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? colour : background ;
//		*r = v ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		b = *(q+4) ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? colour : background ;
//		*r = v ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		v = inv ? colour : background ;
//		*r = v ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v ;
//		*(r+LCD_W+1) = v ;

//		p += LCD_W * 2 ;
//	}
//	return x ;
}
#endif

#ifndef COLOUR_DISPLAY
uint16_t lcd_putcAtt(uint16_t x,uint16_t y,const char c,LcdFlags mode, uint16_t background )
{
	if ( mode & DBLSIZE )
	{
		return lcd_putcAttDblColour( x, y, c, mode ) ;
	}
	if ( c < 22 )		// Move to specific x position (c)*FW
	{
		x = c*FW ;
  	if(mode&DBLSIZE)
		{
			if ( (mode & CONDENSED) )
			{
				x = c*8 ;
			}
		 	else
			{
				x += x ;
			}	 
		}
		return x ;
	}
	
//extern void lcdDrawCharBitmapDma( uint16_t x, uint16_t y, uint8_t chr, uint32_t mode, uint16_t background ) ;
//	if ( (mode & BOLD) == 0 )
//	{
		if ( (mode & BLINK) && BLINK_ON_PHASE )
		{
			mode ^= INVERS ;
		}
		lcdDrawCharBitmapDma( x*2, y*2, c, mode & (INVERS | CONDENSED | BOLD), mode & LUA_CUSTOM_COLOUR ? LcdCustomColour : LcdForeground ) ;
		return (mode & CONDENSED) ? x + FWNUM : x + FW ;
//	}

//	uint16_t *p ;
//  uint8_t *q ;
//	uint8_t mask ;
//	uint16_t v ;
//	uint16_t ov ;

//  register bool inv = false ;
//	if (mode & INVERS) inv = true ;
//	if ( (mode & BLINK) && BLINK_ON_PHASE )
//	{
//		inv = !inv ;
//	}

//	p = ( uint16_t *) CurrentFrameBuffer ;
//	p += x*2 ;
//	p += y * LCD_W * 2 ;

//	x += FW ;
	
////  register bool   inv = (mode & INVERS) ? true : (mode & BLINK ? BLINK_ON_PHASE : false);

//	q = (uint8_t *) &font_5x8_x20_x7f[(c-0x20)*5] ;

//	uint8_t condense = 0 ;

//	if (mode & CONDENSED)
//	{
////		*p = inv ? ~0 : 0;
//		p += 2 ;
//		condense=1;
//		x += FWNUM-FW ;
//	}
	 
//	for ( mask = 1 ; mask ; mask <<= 1 )
//	{
//		uint16_t *r = p ;
//		uint8_t b ;

//		b = *q ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? 0 : background ;
//		*r = v ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		ov = (mode & BOLD) ? v : 0xFFFF ;
//		if ( condense == 0 )
//		{
//			b = *(q+1) ;
//			if ( inv ) b = ~b ;
//			v = ( b & mask ) ? 0 : background ;
//			*r = v & ov ;
//			*(r+1) = v ;
//			*(r+LCD_W) = v & ov ;
//			*(r+LCD_W+1) = v ;
//			r += 2 ;
//			ov = (mode & BOLD) ? v : 0xFFFF ;
//		}
//		b = *(q+2) ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? 0 : background ;
//		*r = v & ov ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v & ov ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		ov = (mode & BOLD) ? v : 0xFFFF ;
//		b = *(q+3) ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? 0 : background ;
//		*r = v & ov ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v & ov ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		ov = (mode & BOLD) ? v : 0xFFFF ;
//		b = *(q+4) ;
//		if ( inv ) b = ~b ;
//		v = ( b & mask ) ? 0 : background ;
//		*r = v & ov ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v & ov ;
//		*(r+LCD_W+1) = v ;
//		r += 2 ;
//		ov = (mode & BOLD) ? v : 0xFFFF ;
//		v = ( inv ) ? 0 : background ;
//		*r = v & ov ;
//		*(r+1) = v ;
//		*(r+LCD_W) = v & ov ;
//		*(r+LCD_W+1) = v ;
//		p += LCD_W * 2 ;
//	}
//	return x ;
}
#endif


//uint16_t lcd_putcAttSmall(uint16_t x,uint16_t y,const char c,uint8_t mode, uint16_t colour)
//{
//	uint16_t *p ;
//  uint8_t *q ;
//	uint8_t mask ;

//	p = ( uint16_t *) CurrentFrameBuffer ;
//	p += x ;
//	p += y * LCD_W ;
//	// p -> char position
//	q = (uint8_t *) &font_5x8_x20_x7f[(c-0x20)*5] ;
//	for ( mask = 1 ; mask ; mask <<= 1 )
//	{
//		if ( *q & mask )
//		{
//			*p = colour ;
//		}
//		if ( *(q+1) & mask )
//		{
//			*(p+1) = colour ;
//		}
//		if ( *(q+2) & mask )
//		{
//			*(p+2) = colour ;
//		}
//		if ( *(q+3) & mask )
//		{
//			*(p+3) = colour ;
//		}
//		if ( *(q+4) & mask )
//		{
//			*(p+4) = colour ;
//		}
//		p += LCD_W ;
//	}
//	return x+6 ;
//}

#ifndef COLOUR_DISPLAY
void lcd_putsnAttColour( uint16_t x, uint16_t y, const char * s,uint8_t len,LcdFlags mode, uint16_t colour, uint16_t bgColour )
{
	register char c ;
  while(len!=0) {
    c = *s++ ;
#ifdef BOOT
		if ( c == 0 )
		{
			break ;			
		}
#endif

    x = lcd_putcAttColour( x, y, c, mode, colour, bgColour ) ;
    len--;
  }
}
#endif

#else
static uint8_t *dispBufAddress( coord_t x, coord_t y )
{
#if (DISPLAY_W==128)
  return &DISPLAY_START[ (y & 0xF8) * 16 + x ];
#else  
	return &DISPLAY_START[ y / 8 * DISPLAY_W + x ];
#endif
}

// invers: 0 no 1=yes 2=blink
 #ifndef PROP_TEXT
uint8_t lcd_putcAtt(coord_t x,coord_t y,const char c,uint8_t mode)
{
	register int32_t i ;
	register uint8_t *p    = dispBufAddress( x, y ) ;
#if PCBX9D
	if ( x > 211-X9D_OFFSET )
	{
		p -= DISPLAY_W ;		
	}
#endif
	if ( c < 22 )		// Move to specific x position (c)*FW
	{
		x = c*FW ;
  	if(mode&DBLSIZE)
		{
			if ( (mode & CONDENSED) )
			{
				x = c*8 ;
			}
		 	else
			{
				x += x ;
			}	 
		}
		return x ;
	}
	x += FW ;
  register uint8_t    *q ;
	if( c < 0xC0 )
	{
		q = (uint8_t *) &font_5x8_x20_x7f[(c-0x20)*5] ;
	}
	else
	{
		if ( ExtraFont )
		{
			q = (uint8_t *) &ExtraFont[(c-0xC0)*5] ;
		}
		else
		{
			q = (uint8_t *) &font_5x8_x20_x7f[0] ;
		}
	}
  register bool inv = false ;
	if (mode & INVERS) inv = true ;
	if ( (mode & BLINK) && BLINK_ON_PHASE )
	{
		inv = !inv ;
	}
  
	if(mode&DBLSIZE)
  {
		uint32_t doNormal = 1 ;
	  unsigned char c_mapped = c ;

		if ( (mode & CONDENSED) )
		{
			doNormal = 0 ;
			
			if ( doNormal == 0 )
			{
				if ( (c!=0x2E)) x+=8-FW; //check for decimal point
			/* each letter consists of 8 top bytes followed by
	 		* five bottom by 8 bottom bytes (16 bytes per 
	 		* char) */
				if( c < 0xC0 )
				{
					c_mapped = c - 0x20 ;
					q = (uint8_t *) &font_12x8[c_mapped*14] ;
				}
				else
				{
					q = (uint8_t *) &font_12x8[0] ;
				}
    		
				for( i=7 ; i>=0 ; i-- )
				{
					uint8_t b1 ;
					uint8_t b3 ;

  		  	/*top byte*/
   		  	b1 = *q ;
  		  	/*bottom byte*/
   		  	b3 = *(q+7) ;
   		  	q++;
					if ( i == 0 )
					{
						b1 = 0 ;
						b3 = 0 ;
					}
    		  if(inv)
					{
				    b1=~b1;
				    b3=~b3;
    		  }

    		  if(&p[DISPLAY_W+1] < DISPLAY_END)
					{
    		    p[0]=b1;
    		    p[DISPLAY_W] = b3;
    		    p+=1;
    		  }
    		}
			}
		}
		if ( doNormal )
		{
			if ( (c!=0x2E)) x+=FW; //check for decimal point
		/* each letter consists of ten top bytes followed by
	 	* five bottom by ten bottom bytes (20 bytes per 
	 	* char) */
	  	unsigned char c_mapped ;
			if( c < 0xC0 )
			{
				c_mapped = c - 0x20 ;
				q = (uint8_t *) &font_10x16_x20_x7f[(c_mapped)*20] ;
			}
			else
			{
				if ( ExtraBigFont )
				{
					q = (uint8_t *) &ExtraBigFont[(c-0xC0)*10] ;
				}
				else
				{
					q = (uint8_t *) &font_10x16_x20_x7f[0] ;
				}
			}
    	for( i=5 ; i>=0 ; i-- )
			{
				uint8_t b1 ;
				uint8_t b3 ;
				uint8_t b2 ;
				uint8_t b4 ;

				if( c < 0xC0 )
				{
	  	  	/*top byte*/
    	  	b1 = i>0 ? *q : 0;
	  	  	/*bottom byte*/
    	  	b3 = i>0 ? *(q+10) : 0;
	  	  	/*top byte*/
    	  	b2 = i>0 ? *(++q) : 0;
	  	  	/*bottom byte*/
    	  	b4 = i>0 ? *(q+10) : 0;
    	  	q++;
				}
				else
				{
	  	  	/*top byte*/
    	  	b1 = i>0 ? *q++ : 0 ;
	  	  	/*bottom byte*/
    	  	b3 = i>0 ? *q++ : 0 ;
	  	  	/*top byte*/
    	  	b2 = i>0 ? *q++ : 0 ;
	  	  	/*bottom byte*/
    	  	b4 = i>0 ? *q++ : 0 ;
				}
    	  if(inv)
				{
			    b1=~b1;
			    b2=~b2;
			    b3=~b3;
			    b4=~b4;
    	  }

    	  if(&p[DISPLAY_W+1] < DISPLAY_END)
				{
    	    p[0]=b1;
    	    p[1]=b2;
    	    p[DISPLAY_W] = b3;
    	    p[DISPLAY_W+1] = b4;
    	    p+=2;
    	  }
    	}
		}
  }
  else
  {
		uint8_t condense=0;

		if (mode & CONDENSED)
		{
			*p = inv ? ~0 : 0;
			p += 1 ;
			condense=1;
			x += FWNUM-FW ;
		}

		y &= 7 ;
		if ( y )
		{ // off grid
    	for( i=5 ; i!=0 ; i-- )
			{
      	uint16_t b = *q++ ;
				if ( inv )
				{
					b = ~b & 0x00FF ;
				}
				b <<= y ;
      	if(p<DISPLAY_END) *p ^= b ;
      	if(&p[DISPLAY_W] < DISPLAY_END)
				{
	        p[DISPLAY_W] ^= b >> 8 ;
				}
				p += 1 ;
			}
			if ( inv )
			{
				uint16_t b = 0xFF ;
     		if(p<DISPLAY_END) *p ^= ( b <<= y) ;
     		if(&p[DISPLAY_W] < DISPLAY_END) p[DISPLAY_W] ^= b >> 8 ;
			}
		}
		else
		{
			uint8_t oldb = 0 ;
    	for( i=5 ; i!=0 ; i-- )
			{
    	  uint8_t b = *q++ ;
  		  if (condense && i==4)
				{
    	    /*condense the letter by skipping column 4 */
    	    continue;
    	  }
    	  if(p<DISPLAY_END) *p++ = inv ? ~(b|oldb) : (b|oldb) ;
				if (mode & BOLD)
				{
					oldb = b ;
				}
    	}
    	if(p<DISPLAY_END) *p++ = inv ? ~oldb : oldb ;
		}
  }
	return x ;
}
 #endif

#ifndef PROP_TEXT
uint8_t lcd_putcSmall( coord_t x, coord_t y, uint8_t c, LcdFlags mode )
{
	register int32_t i ;
	register uint8_t *p = dispBufAddress( x, y ) ;
  register bool inv = false ;
#if PCBX9D
	if ( x > 211-X9D_OFFSET )
	{
		p -= DISPLAY_W ;		
	}
#endif
	x += 5 ;
  register uint8_t *q ;
	if( c < 0x80 )
	{
		q = (uint8_t *) &font_04_06[(c-0x20)*5] ;
	}
	else
	{
		q = (uint8_t *) &font_04_06[0] ;
	}
	if (mode & INVERS) inv = true ;
	if ( (mode & BLINK) && BLINK_ON_PHASE )
	{
		inv = !inv ;
	}

	// Assume off grid
	y &= 7 ;
  for( i = 5 ; i != 0 ; i -= 1 )
	{
    uint16_t b = *q++ ;
		if ( inv )
		{
			b ^= 0x3F ;
		}
		b <<= y ;
   	if(p<DISPLAY_END) *p ^= b ;
    if(&p[DISPLAY_W] < DISPLAY_END)
		{
	    p[DISPLAY_W] ^= b >> 8 ;
		}
		p += 1 ;
	}
	return x ;
}
#endif // PROP_TEXT

#endif

// Puts sub-string from string options
// First byte of string is sub-string length
// idx is index into string (in length units)
// Output length characters
//#if defined(PCBX12D) || defined(PCBX10)
//void lcd_putsAttIdxColour(uint8_t x,uint8_t y,const char * s,uint8_t idx,uint8_t att, uint16_t colour, uint16_t background )
//{
//	uint8_t length ;
//	length = *s++ ;

//  lcd_putsnAttColour(x,y,s+length*idx,length,att, colour, background ) ;
//}
//#endif

#if defined(PCBX12D) || defined(PCBX10)
#else
 #ifndef PROP_TEXT
void lcd_putsAttIdx(coord_t x,coord_t y,const char * s,uint8_t idx,LcdFlags att)
{
	uint8_t length ;
	length = *s++ ;
	if ( att & LUA_RIGHT)
	{
		x -= length*FW ;
		att &= ~LUA_RIGHT ;
	}
  PUTS_ATT_N(x,y,s+length*idx,length,att) ;
}
 #endif
#endif

//void lcd_putsAttIdx_right( uint8_t y,const prog_char * s,uint8_t idx,uint8_t att)
//{
//	uint8_t x = 20 - pgm_read_byte(s) ;
//	lcd_putsAttIdx( x, y, s, idx, att ) ;
//}

#if defined(PCBX12D) || defined(PCBX10) || defined(PROP_TEXT)
#else
void lcd_putsnAtt(coord_t x,coord_t y, const char * s,uint8_t len,LcdFlags mode)
{
#ifndef TOUCH
 #if defined(PCBX12D) || defined(PCBX10)
	if ( mode & LUA_SMLSIZE )
	{
		lcd_putsnSmall( x, y, (uint8_t *)s, len, LcdForeground ) ;		
		return ;
	}
 #endif
#endif
	 
	register char c ;
  while(len!=0) {
    c = *s++ ;
#ifdef BOOT
		if ( c == 0 )
		{
			break ;			
		}
#endif

//#ifndef TOUCH
//		if ( mode & LUA_SMLSIZE )
//		{
// #if defined(PCBX12D) || defined(PCBX10)
////			lcdDrawCharSmall( x*2, y*2, c, mode ) ;
////			x += 6 ;
//			x = lcd_putcAtt(x, y, c, mode);
// #else			
//			x = lcd_putcSmall( x, y, c, mode ) ;
// #endif
//		}
//		else
//#endif
		{
#ifndef TOUCH
			if ( mode & LUA_SMLSIZE )
			{
				x = lcd_putcSmall( x, y, c, mode ) ;
			}
			else
#endif
			{
				x = lcd_putcAtt(x, y, c, mode);
			}
		}
    len--;
  }
}
#endif

#if defined(PCBX12D) || defined(PCBX10) || defined(PROP_TEXT)
#else
void lcd_putsn_P(coord_t x,coord_t y,const char * s,uint8_t len)
{
  lcd_putsnAtt( x,y,s,len,0);
}
#endif

//#ifndef COLOUR_DISPLAY
//#if defined(PCBX12D) || defined(PCBX10)
//uint8_t lcd_putsAttColour( uint8_t x, uint8_t y, const char *s, LcdFlags mode, uint16_t colour, uint16_t background )
//{
//  while(1)
//	{
//    char c = *s++ ;
//    if(!c) break ;
//		if ( c == 31 )
//		{

//			if ( (y += FH) >= 64 )	// Screen height
//			{
//				break ;
//			}	
//			x = DisplayOffset ;
//		}
//		else
//		{
//    	x = lcd_putcAttColour(x,y,c,mode, colour, background ) ;
//		}
//  }
//  return x;
//}
//#endif
//#endif
 
 
 #ifndef PROP_TEXT
#ifndef COLOUR_DISPLAY
uint8_t lcd_putsAtt( coord_t x, coord_t y, const char *s, LcdFlags mode )
{
	if ( mode & LUA_RIGHT )
	{
		uint32_t length = strlen(s) ;
		uint32_t width = ( mode & LUA_SMLSIZE ) ? 5 : FW ;
  	if(mode&DBLSIZE)
		{
			if ( (mode & CONDENSED) )
			{
				width = 8 ;
			}
		 	else
			{
				width += width ;
			}	 
		}
		length *= width ;
		x -= length ;
	}
  while(1)
	{
    char c = *s++ ;
    if(!c) break ;
		if ( c == 31 )
		{

#if defined(PCBX12D) || defined(PCBX10)
			if ( (y += FH) >= 272/2 )	// Screen height
			{
				break ;
			}	
#else
			if ( (y += FH) >= DISPLAY_H )	// Screen height
			{
				break ;
			}	
#endif
			
//#if defined(PCBX12D) || defined(PCBX10)
//			x = DisplayOffset ;
//#else
			x = 0 ;
//#endif
		}
		else
		{
//#if defined(PCBX12D) || defined(PCBX10)
//			uint8_t offset = 0 ;
//			if ( c < 22 )
//			{
//				offset = DisplayOffset ;
//			}
//#endif
#if defined(PCBX12D) || defined(PCBX10)
		x = lcd_putcAtt(x, y, c, mode);
#else
		if ( mode & LUA_SMLSIZE )
		{
			x = lcd_putcSmall( x, y, c, mode ) ;
		}
		else
		{
			x = lcd_putcAtt(x, y, c, mode);
		}
#endif
//#if defined(PCBX12D) || defined(PCBX10)
//			x += offset ;
//#endif
		}
  }
  return x;
}
 #endif

#if defined(PCBX12D) || defined(PCBX10)
#else
 #ifndef PROP_TEXT
void lcd_puts_Pleft( coord_t y, const char *s )
{
#if defined(PCBX12D) || defined(PCBX10)
  lcd_putsAtt( 0, y, s, 0);
#else
  lcd_putsAtt( 0, y, s, 0);
#endif
}
 #endif
 #endif
// This routine skips 'skip' strings, then displays the rest
//void lcd_puts_Pskip(uint8_t y,const char * s, uint8_t skip)
//{
//	while ( skip )
//	{
//    char c = *s++ ;
//    if(!c) return ;
//		if ( c == 31 )
//		{
//			skip -= 1 ;
//		}
//	}
//  lcd_putsAtt( 0, y, s, 0);
//}

 #ifndef PROP_TEXT
#ifndef COLOUR_DISPLAY
void lcd_puts_P( coord_t x, coord_t y, const char *s )
{
  lcd_putsAtt( x, y, s, 0);
}
#endif
 #endif
#endif

#if defined(PCBX7ACCESS)
//void lcd_outhex8(uint16_t x,uint8_t y,uint32_t val)
//{
//	uint8_t i ;
//  x += FWNUM*8 ;
//  for(i=0 ; i<8 ; i += 1)
//  {
//    x -= FWNUM ;
//    char c = val & 0x0F ;
//    c = c>9 ? c+'A'-10 : c+'0' ;
//    lcd_putcAtt(x,y,c,c>='A'?CONDENSED:0) ;
//    val >>= 4 ;
//  }
//}
#endif

#if defined(PCBX12D) || defined(PCBX10) || defined(PCB9XT)
//void lcd_outhex8(uint16_t x,uint8_t y,uint32_t val)
//{
//	uint8_t i ;
//  x += FWNUM*8 ;
//  for(i=0 ; i<8 ; i += 1)
//  {
//    x -= FWNUM ;
//    char c = val & 0x0F ;
//    c = c>9 ? c+'A'-10 : c+'0' ;
//    lcd_putcAtt(x,y,c,c>='A'?CONDENSED:0) ;
//    val >>= 4 ;
//  }
//}
#endif

#ifndef PROP_TEXT
 #if defined(PCBX12D) || defined(PCBX10)
 #else
#if defined(PCBX12D) || defined(PCBX10) || defined(PCB9XT)
void lcd_outhex4(uint16_t x,uint8_t y,uint16_t val)
#else
void lcd_outhex4(coord_t x,coord_t y,uint16_t val)
#endif
{
	uint32_t i ;
  x+=FWNUM*4;
  for(i=0; i<4; i++)
  {
    x-=FWNUM;
    char c = val & 0xf;
    c = c>9 ? c+'A'-10 : c+'0';
    PUTC_ATT(x,y,c,c>='A'?CONDENSED:0);
    val>>=4;
  }
}
 #endif
#endif // PROP_TEXT
//void lcd_outhex2(uint8_t x,uint8_t y,uint8_t val)
//{
//  register int i ;
//  x+=FWNUM*2;
//  for(i=0; i<2; i++)
//  {
//    x-=FWNUM;
//    char c = val & 0xf;
//    c = c>9 ? c+'A'-10 : c+'0';
//    lcd_putcAtt(x,y,c,c>='A'?CONDENSED:0);
//    val>>=4;
//  }
//}

#if defined(PCBX12D) || defined(PCBX10)
//void lcd_outdez( uint16_t x, uint16_t y, int16_t val, uint16_t background )
//{
//  lcd_outdezAtt(x,y,val,0, background );
//}

//void lcd_outdezAtt( uint16_t x, uint16_t y, int16_t val, uint16_t mode, uint16_t background )
//{
//  lcd_outdezNAtt( x,y,val,mode,5, LcdForeground, background );
//}
#else
 #ifndef PROP_TEXT
void lcd_outdez( coord_t x, coord_t y, int16_t val )
{
  lcd_outdezAtt(x,y,val,0);
}

void lcd_outdezAtt( coord_t x, coord_t y, int16_t val, LcdFlags mode )
{
  lcd_outdezNAtt( x,y,val,mode,5);
}
 #endif
#endif

#if defined(PCBX12D) || defined(PCBX10)
//void lcd_2_digits( uint16_t x, uint16_t y, uint8_t value, LcdFlags attr, uint16_t colour, uint16_t background )
//{
//	PUTS_NUM_N( x, y, value, attr, 2 ) ; // , colour, background ) ;
//}
#else
 #ifndef PROP_TEXT
void lcd_2_digits( coord_t x, coord_t y, uint8_t value, LcdFlags attr )
{
	PUTS_NUM_N( x, y, value, attr + LEADING0, 2 ) ;
}
 #endif // PROP_TEXT
#endif

#define PREC(n) ((n&0x20) ? ((n&0x10) ? 2 : 1) : 0)

#if defined(PCBX12D) || defined(PCBX10)
//uint8_t lcd_outdezNAtt( uint16_t x, uint16_t y, int32_t val, uint16_t mode, int8_t len, uint16_t colour, uint16_t background )
#else
 #ifndef PROP_TEXT
uint8_t lcd_outdezNAtt( coord_t x, coord_t y, int32_t val, LcdFlags mode, int8_t len )
{
  uint8_t fw = FWNUM;
  uint8_t prec = PREC(mode);
  int32_t tmp = abs(val);
  uint8_t xn = 0;
  uint8_t ln = 2;
  char c;
  uint8_t xinc ;
	uint8_t fullwidth = 0 ;
	int32_t i ;
	if ( len < 0 )
	{
		fullwidth = 1 ;
		len = -len ;		
	}
#ifndef PCBX12D
#ifndef PCBX10
	if (mode & BOLD)
	{
		fw = FW ;
	}
#endif
#endif
  if (mode & DBLSIZE)
  {
		if ( (mode & CONDENSED) )
		{
    	fw = 8 ;
    	xinc = 8 ;
    	Lcd_lastPos = 8 ;
		}
		else
		{
    	fw += FWNUM ;
    	xinc = 2*FWNUM;
    	Lcd_lastPos = 2*FW;
		}
  }
  else
  {
    xinc = FWNUM ;
    Lcd_lastPos = FW;
  }

  if (mode & LEFT)
	{
    if(val<0)
    {
      x += fw;
    }
    if (tmp >= 1000)
		{
	    if (tmp >= 100000)
  	    x += fw;
	    if (tmp >= 10000)
  	    x += fw;
			x += 3 * fw ;	
		}
		else
		{
	    if (tmp >= 100)
  	    x += fw;
    	if (tmp >= 10)
      	x += fw;
		}
//    if (tmp >= 10000)
//      x += fw;
//    if (tmp >= 1000)
//      x += fw;
//    if (tmp >= 100)
//      x += fw;
//    if (tmp >= 10)
//      x += fw;
    if ( prec )
    {
      if ( prec == 2 )
      {
        if ( tmp < 100 )
        {
          x += fw;
        }
      }
      if ( tmp < 10 )
      {
        x+= fw;
      }
    }
  }
  else
  {
    x -= xinc;
  }
  Lcd_lastPos += x ;

//  if ( prec == 2 )
//  {
//    mode -= LEADING0;  // Can't have PREC2 and LEADING0
//  }

  for ( i=1; i<=len; i++)
	{
    c = (tmp % 10) + '0';
#if defined(PCBX12D) || defined(PCBX10)
		lcd_putcAttColour( x, y, c, mode, colour, background ) ;
#else
		if ( mode & LUA_SMLSIZE )
		{
			lcd_putcSmall( x, y, c, mode ) ;
		}
		else
		{
			lcd_putcAtt(x, y, c, mode);
		}
#endif
    if (prec==i)
		{
      if (mode & DBLSIZE)
			{
        xn = x ;
				if ( (mode & CONDENSED) )
				{
					x -= 1 ;
        	xn = x-1 ;
				}
				else
				{
	        if(c=='2' || c=='3' || c=='1') ln++;
	        uint8_t tn = (tmp/10) % 10;
	        if(tn==2 || tn==4)
					{
	          if (c=='4')
						{
	            xn++;
	          }
	          else
						{
	            xn--; ln++;
	          }
	        }
				}
      }
      else
			{
        x -= 2;
#if defined(PCBX12D) || defined(PCBX10)
				pushPlotType( PLOT_CUSTOM ) ;
				LcdCustomColour = colour ;
        if (mode & INVERS)
          lcd_vline(x+1, y, 7);
        else
          lcd_plot(x+1, y+6);
				popPlotType() ;
#else
				uint8_t oldPlotType = plotType ;
				plotType = PLOT_BLACK ;
        if (mode & INVERS)
				{
          lcd_vline(x+1, y, 8);
				}
				plotType = oldPlotType ;
        lcd_plot(x+1, y+6);
#endif
      }
      if (tmp >= 10)
        prec = 0;
    }
    tmp /= 10;
    if (!tmp)
    {
      if (prec)
      {
        if ( prec == 2 )
        {
          if ( i > 1 )
          {
            prec = 0 ;
          }
        }
        else
        {
          prec = 0 ;
        }
      }
      else if (mode & LEADING0)
			{
				if ( fullwidth == 0 )
				{
        	mode -= LEADING0;
				}
			}
      else
        break;
    }
    x-=fw;
  }
  if (xn)
	{
#if defined(PCBX12D) || defined(PCBX10)
		pushPlotType( PLOT_CUSTOM ) ;
		LcdCustomColour = colour ;
#endif
    lcd_hline(xn, y+2*FH-4, ln);
    lcd_hline(xn, y+2*FH-3, ln);
#if defined(PCBX12D) || defined(PCBX10)
		popPlotType() ;
#endif
  }
#if defined(PCBX12D) || defined(PCBX10)
  if(val<0) lcd_putcAttColour(x-fw,y,'-',mode, colour, background ) ;
#else
  if(val<0)
	{
		if ( mode & 32768 )
		{
			lcd_putcSmall( x, y, '-', mode ) ;
		}
		else
		{
			lcd_putcAtt(x-fw,y,'-',mode);
		}
	}
#endif
	return 0 ;		// Stops compiler creating two sets of POPS, saves flash
}
 #endif
#endif


#ifndef PCBX12D
#ifndef PCBX10
void lcd_write_bits( uint8_t *p, uint8_t mask )
{
  if(p<DISPLAY_END)
	{
		uint8_t temp = *p ;
		if ( plotType != PLOT_XOR )
		{
			temp |= mask ;
		}
		if ( ( plotType != PLOT_BLACK ) && ( plotType != PLOT_COLOUR ) )
		{
			temp ^= mask ;
		}
		*p = temp ;
	}
}
#endif
#endif

#if defined(PCBX12D) || defined(PCBX10)
void lcd_plot( uint16_t x, uint16_t y )
{
#ifdef INVERT_DISPLAY
	x = (LCD_W/2-1) - x ;
	y = (LCD_H/2-1) - y ;
	if ( y >= LCD_H/2 )
	{
//		LcdBits |= 32 ;
//		LcdErrors += 1 ;
		y = 0 ;
	}
#endif
	uint16_t *p = ( uint16_t *) CurrentFrameBuffer ;
	p += x*2 ;
	p += y * LCD_W * 2 ;

#ifdef INVERT_DISPLAY
//	p += 1 ;
//	p += LCD_W ;
#endif
	 
	if ( plotType == PLOT_XOR )
	{
		*p ^= 0xFFFF ;
		*(p+1) ^= 0xFFFF ;
		*(p+LCD_W) ^= 0xFFFF ;
		*(p+LCD_W+1) ^= 0xFFFF ;
	}
	else
	{
		uint16_t colour = 0 ;
		if ( plotType == PLOT_WHITE )
		{
			colour = 0xFFFF ;
		}
		else if ( plotType == PLOT_COLOUR )
		{
			colour = LcdForeground ;
		}
		else if ( plotType == PLOT_CUSTOM )
		{
			colour = LcdCustomColour ;
		}
		*p = colour ;
		*(p+1) = colour ;
		*(p+LCD_W) = colour ;
		*(p+LCD_W+1) = colour ;
	}
}

void lcd_hlineStip( uint16_t x, uint16_t y, int16_t w, uint8_t pat )
{
  if(w<0) {x+=w; w=-w;}

#ifdef INVERT_DISPLAY
	x = (LCD_W/2-1) - x ;
	y = (LCD_H/2-1) - y ;
	if ( y >= LCD_H/2 )
	{
		y = 0 ;
	}
#endif
	uint16_t *p = ( uint16_t *) CurrentFrameBuffer ;
	p += x*2 ;
	p += y * LCD_W * 2 ;

#ifdef INVERT_DISPLAY
//	p += 1 ;
//	p += LCD_W ;
#endif

	while(w)
	{
//    if ( p>=DISPLAY_END)
//    {
//      break ;			
//    }
    if(pat&1)
		{
			if ( plotType == PLOT_XOR )
			{
				*p ^= 0xFFFF ;
				*(p+1) ^= 0xFFFF ;
				*(p+LCD_W) ^= 0xFFFF ;
				*(p+LCD_W+1) ^= 0xFFFF ;
			}
			else
			{
				uint16_t colour = 0 ;
				if ( plotType == PLOT_WHITE )
				{
					colour = 0xFFFF ;
				}
				else if ( plotType == PLOT_COLOUR )
				{
					colour = LcdForeground ;
				}
				else if ( plotType == PLOT_CUSTOM )
				{
					colour = LcdCustomColour ;
				}
				*p = colour ;
				*(p+1) = colour ;
				*(p+LCD_W) = colour ;
				*(p+LCD_W+1) = colour ;
			}
      pat = (pat >> 1) | 0x80;
    }
		else
		{
      pat = pat >> 1;
    }
    w -= 1 ;
#ifdef INVERT_DISPLAY
    p -= 2 ;
#else
    p += 2 ;
#endif
  }
}

void lcd_hline( uint16_t x, uint16_t y, int16_t w )
{
  lcd_hlineStip(x,y,w,0xff);
}

void lcd_vline( coord_t x, coord_t y, int8_t h )
{
	int16_t y1 ;
	y1 = y ;
  if (h<0) { y1+=h; h=-h; }
	while ( y1 < 0 )
	{
		y1 += 1 ;
		h -= 1 ;
	}
	if ( h <= 0 )
	{
		return ;
	}
	if ( y1 + h >= 272/2 )
	{
		h -= 272/2 - y1 ;
	}
	if ( h <= 0 )
	{
		return ;
	}

#ifdef INVERT_DISPLAY
	x = (LCD_W/2-1) - x ;
	y1 = (LCD_H/2-1) - y1 ;
	if ( y1 >= LCD_H/2 )
	{
//		LcdBits |= 64 ;
//		LcdErrors += 1 ;
		y1 = 0 ;
	}
#endif
	uint16_t *p = ( uint16_t *) CurrentFrameBuffer ;
	p += x*2 ;
	p += y1 * LCD_W * 2 ;

#ifdef INVERT_DISPLAY
//	p += 1 ;
//	p += LCD_W ;
#endif
  
  while( h )
	{
		h -= 1 ;
		if ( plotType == PLOT_XOR )
		{
			*p ^= 0xFFFF ;
			*(p+1) ^= 0xFFFF ;
			*(p+LCD_W) ^= 0xFFFF ;
			*(p+LCD_W+1) ^= 0xFFFF ;
		}
		else
		{
			uint16_t colour = 0 ;
			if ( plotType == PLOT_WHITE )
			{
				colour = 0xFFFF ;
			}
			else if ( plotType == PLOT_COLOUR )
			{
				colour = LcdForeground ;
			}
			else if ( plotType == PLOT_CUSTOM )
			{
				colour = LcdCustomColour ;
			}
			*p = colour ;
			*(p+1) = colour ;
			*(p+LCD_W) = colour ;
			*(p+LCD_W+1) = colour ;
		}
#ifdef INVERT_DISPLAY
		p -= LCD_W * 2 ;
#else
		p += LCD_W * 2 ;
#endif
  }
}


#else

void lcd_plot( coord_t x, coord_t y )
{
	uint8_t *p = dispBufAddress( x, y ) ;
//#ifdef GREY_SCALE
//  register uint8_t *p   = &DISPLAY_START[ y / 2 * DISPLAY_W + x ];
//#else
//  register uint8_t *p   = &DISPLAY_START[ y / 8 * DISPLAY_W + x ];
//#endif
#ifdef GREY_SCALE
	lcd_write_bits( p, (y&1) ? 0xF0 : 0x0F ) ;
#else
	lcd_write_bits( p, BITMASK(y%8) ) ;
#endif
}

void lcd_hlineStip( coord_t x, coord_t y, int16_t w, uint8_t pat )
{
  if(w<0) {x+=w; w=-w;}
#ifdef GREY_SCALE
  register uint8_t *p = &DISPLAY_START[ y / 2 * DISPLAY_W + x ] ;
	register uint8_t msk = (y&1) ? 0xF0 : 0x0F ;
#else
	uint8_t *p = dispBufAddress( x, y ) ;
	register uint8_t msk = BITMASK(y%8) ;
#endif  
  while(w)
	{
    if ( p>=DISPLAY_END)
    {
      break ;			
    }
    if(pat&1)
		{
			lcd_write_bits( p, msk ) ;
      pat = (pat >> 1) | 0x80;
    }
		else
		{
      pat = pat >> 1;
    }
    w--;
    p++;
  }
}
#endif // X12D

#if defined(PCBX12D) || defined(PCBX10)
void lcdDrawFilledRect( coord_t x, coord_t y, uint16_t w, uint16_t h, uint8_t pat, LcdFlags att )
{
	uint8_t oldPlotType = plotType ;
	uint32_t i ;
  
	for ( i=y; i<y+h; i++ )
	{
  	lcd_hlineStip( x, i, w, 0xff ) ;
	}
	plotType = oldPlotType ;
}
#else
void lcdDrawFilledRect( coord_t x, coord_t y, uint8_t w, uint8_t h, uint8_t pat, LcdFlags att )
{
	uint8_t oldPlotType = plotType ;

	uint32_t i ;
	plotType = PLOT_XOR ;
	if ( att & FORCE )
	{
		plotType = PLOT_BLACK ;
	}
	if ( att & ERASE )
	{
		plotType = PLOT_WHITE ;
	}
  if ( x+w > LCD_W)
	{
		w = LCD_W - x ;
	}

  for ( i=y; i<y+h; i++ )
	{
  	lcd_hlineStip( x, i, w, 0xff ) ;
		
//    if ((att&ROUND) && (i==y || i==y+h-1))
//      lcdDrawHorizontalLine(x+1, i, w-2, pat, att);
//    else
//      lcdDrawHorizontalLine(x, i, w, pat, att);
//    pat = (pat >> 1) + ((pat & 1) << 7);
  }
	plotType = oldPlotType ;
}
#endif


void lcd_hbar( coord_t x, coord_t y, uint8_t w, uint8_t h, uint8_t percent )
{
	uint8_t solid ;
	if ( percent > 100 )
	{
		percent = 100 ;
	}
	solid = (w-2) * percent / 100 ;
#if defined(PCBX12D) || defined(PCBX10)
	lcd_rectColour( x, y, w, h, plotType == PLOT_BLACK ? LCD_BLACK : LcdForeground ) ;
//	lcd_rect( x, y, w, h ) ;
#else
	lcd_rect( x, y, w, h ) ;
#endif

	if ( solid )
	{
		w = y + h - 1 ;
		y += 1 ;
		x += 1 ;
		while ( y < w )
		{
 			lcd_hline(x, y, solid ) ;
			y += 1 ;			
		}
	}
}

#if defined(PCBX12D) || defined(PCBX10)
void lcd_char_inverse( uint16_t x, uint16_t y, uint16_t w, uint8_t blink, uint8_t h )
{
	if ( blink && BLINK_ON_PHASE )
	{
		return ;
	}
#ifdef INVERT_DISPLAY
	x = (LCD_W/2-1) - x ;
	y = (LCD_H/2-1) - y - (h-1) ;
	if ( y >= LCD_H/2 )
	{
//		LcdBits |= 128 ;
//		LcdErrors += 1 ;
		y = 0 ;
	}
#endif
	x *= 2 ;
	w *= 2 ;
	uint16_t *p = ( uint16_t *) CurrentFrameBuffer ;
	p += x ;
	p += y * LCD_W * 2 ;
	
#ifdef INVERT_DISPLAY
	p -= w - 2 ;
//	p -= LCD_W*16 ;
#endif
	uint16_t end = x + w ;

	h *= 2 ;
	
	uint32_t i ;
	uint16_t sx = x ;
	for ( i = 0 ; i < h ; i += 1 )
	{
		uint32_t *q = (uint32_t *)p ;
		x = sx ;
		while ( x < end )
		{
			*q ^= 0xFFFFFFFF ;
			q += 1 ;
			x += 2 ;
		}
		p += LCD_W ;
	}
	
//	while ( x < end )
//	{
//		uint32_t i ;
//		uint32_t *q = (uint32_t *)p ;
////		uint16_t *q = p ;
//		for ( i = 0 ; i < h ; i += 1 )
//		{
//			*q ^= 0xFFFFFFFF ;
//			q += LCD_W/2 ;
//		}
//		p += 2 ;
//		x += 2 ;
//	}
}
#else
// Reverse video 8 pixels high, w pixels wide
// Vertically on an 8 pixel high boundary
void lcd_char_inverse( coord_t x, coord_t y, uint8_t w, uint8_t blink )
{
	if ( blink && BLINK_ON_PHASE )
	{
		return ;
	}
	uint8_t end = x + w ;
	uint8_t *p = dispBufAddress( x, y ) ;
//#ifdef GREY_SCALE
//  register uint8_t *p = &DISPLAY_START[ y / 2 * DISPLAY_W + x ] ;
//#else
//  register uint8_t *p = &DISPLAY_START[ y / 8 * DISPLAY_W + x ] ;
//#endif  

	y &= 7 ;
	if ( y )
	{ // off grid
		while ( x < end )
		{
#ifdef GREY_SCALE
			*p ^= 0xFF ;
			*(p+DISPLAY_W) ^= 0xFF ;
			*(p+DISPLAY_W*2) ^= 0xFF ;
			*(p+DISPLAY_W*3) ^= 0xFF ;
			p += 1 ;
#else
     	uint16_t b = 0xFF ;
			b <<= y ;
			if(p<DISPLAY_END) *p ^= b ;
	    if(&p[DISPLAY_W] < DISPLAY_END) p[DISPLAY_W] ^= b >> 8 ;
			p += 1 ;
#endif  
			x += 1 ;
		}
	}
	else
	{
		while ( x < end )
		{
#ifdef GREY_SCALE
			*p ^= 0xFF ;
			*(p+DISPLAY_W) ^= 0xFF ;
			*(p+DISPLAY_W*2) ^= 0xFF ;
			*(p+DISPLAY_W*3) ^= 0xFF ;
			p += 1 ;
#else
			if(p<DISPLAY_END) *p++ ^= 0xFF ;
#endif  
			x += 1 ;
		}
	}
}
#endif

//extern uint16_t LCDLastOp ;
#if defined(PCBX12D) || defined(PCBX10)
void lcd_rectColour(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint16_t colour )
{
	lcdDrawSolidFilledRectDMA( x*2, y*2, 2, h*2, colour) ;
	lcdDrawSolidFilledRectDMA( (x+w-1)*2, y*2, 2, h*2, colour) ;
	lcdDrawSolidFilledRectDMA( (x+1)*2, (y+h-1)*2, (w-2)*2, 2, colour) ;
	lcdDrawSolidFilledRectDMA( (x+1)*2, y*2, (w-2)*2, 2, colour) ;
}

void lcdHiresRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t colour )
{
	lcdDrawSolidFilledRectDMA( x, y, 1, h, colour) ;
	lcdDrawSolidFilledRectDMA( (x+w-1), y, 1, h, colour) ;
	lcdDrawSolidFilledRectDMA( (x+1), (y+h-1), (w-2), 1, colour) ;
	lcdDrawSolidFilledRectDMA( (x+1), y, (w-2), 1, colour) ;
}

void lcdHiresHline( uint16_t x, uint16_t y, uint16_t w, uint16_t colour )
{
	lcdDrawSolidFilledRectDMA( (x), (y), (w), 1, colour) ;
}

void lcdHiresVline( uint16_t x, uint16_t y, uint16_t h, uint16_t colour )
{
	lcdDrawSolidFilledRectDMA( x, y, 1, h, colour) ;
}

void lcdHiresHbar( uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t percent, uint16_t colour )
{
	uint8_t solid ;
	if ( percent > 100 )
	{
		percent = 100 ;
	}
	solid = w * percent / 100 ;

	if ( solid )
	{
		lcdDrawSolidFilledRectDMA( x+1, y+1, solid, h, colour) ;
	}
}

//uint16_t lcdHiresPutsTransparent( uint16_t x, uint16_t y, char *s, uint16_t colour )
//{
//  while(1)
//	{
//    char c = *s++ ;
//    if(!c) break ;
//		if ( c == 31 )
//		{

//#if defined(PCBX12D) || defined(PCBX10)
//			if ( (y += FH*2) >= 272 )	// Screen height
//			{
//				break ;
//			}	
//#else
//			if ( (y += FH) >= DISPLAY_H )	// Screen height
//			{
//				break ;
//			}	
//#endif
//			x = DisplayOffset ;
//		}
//		else
//		{
//			lcdDrawCharBitmapDma( x, y, c, 0, colour ) ;
//			x += 12 ;
//		}
//  }
//  return x ;
//}

void lcdHiresPlot( uint16_t x, uint16_t y, uint16_t h, uint16_t colour )
{
#ifdef INVERT_DISPLAY
	x = (LCD_W/2-1) - x ;
	y = (LCD_H/2-1) - y ;
	if ( y >= LCD_H/2 )
	{
		y = 0 ;
	}
#endif
	uint16_t *p = ( uint16_t *) CurrentFrameBuffer ;
	p += x ;
	p += y * LCD_W ;

	if ( plotType == PLOT_XOR )
	{
		*p ^= colour ;
	}
	else
	{
		*p = colour ;
	}
}

#endif

void lcd_rect(coord_t x, coord_t y, uint8_t w, uint8_t h )
{
	uint8_t oldPlotType = plotType ;
	if ( oldPlotType != PLOT_WHITE )
	{
		plotType = PLOT_BLACK ;
	}
#if defined(PCBX12D) || defined(PCBX10)
	uint16_t colour = (plotType == PLOT_BLACK) ? LCD_BLACK : LcdBackground ;
#ifdef WHERE_TRACK
notePosition('0') ;
#endif
	//LCDLastOp = 'E' ;
	lcdDrawSolidFilledRectDMA( x*2, y*2, 2, h*2, colour) ;
//	LCDLastOp = 'F' ;
	lcdDrawSolidFilledRectDMA( (x+w-1)*2, y*2, 2, h*2, colour) ;
//	LCDLastOp = 'G' ;
	lcdDrawSolidFilledRectDMA( (x+1)*2, (y+h-1)*2, (w-2)*2, 2, colour) ;
//	LCDLastOp = 'H' ;
	lcdDrawSolidFilledRectDMA( (x+1)*2, y*2, (w-2)*2, 2, colour) ;
#else
  lcd_vline(x, y, h ) ;
	if ( w > 1 )
	{
  	lcd_vline(x+w-1, y, h ) ;
	}
 	lcd_hline(x+1, y+h-1, w-2 ) ;
 	lcd_hline(x+1, y, w-2 ) ;
#endif
	plotType = oldPlotType ;
}

void lcd_hline( coord_t x, coord_t y, int8_t w )
{
  lcd_hlineStip(x,y,w,0xff);
}

#ifndef PCBX12D
#ifndef PCBX10
void lcd_vline( coord_t x, coord_t y, int8_t h )
{
  if (h<0) { y+=h; h=-h; }

#ifdef GREY_SCALE
  register uint8_t *p   = &DISPLAY_START[ y / 2 * DISPLAY_W + x ];
  y &= 0x01 ;
	if ( y )
	{
    uint8_t msk = 0x0F ;
    h -= 1 ;
		lcd_write_bits( p, msk ) ;
    p += DISPLAY_W ;
	}
    
  while( h >= 2 )
	{
		h -= 2 ;
		lcd_write_bits( p, 0xFF ) ;
    p += DISPLAY_W ;
  }
	if ( h > 0 )
	{
  	lcd_write_bits( p, 0xF0 ) ;
	}
#else
	uint8_t *p = dispBufAddress( x, y ) ;
  y &= 0x07 ;
	if ( y )
	{
    uint8_t msk = ~(BITMASK(y)-1) ;
    h -= 8-y ;
    if (h < 0)
      msk -= ~(BITMASK(8+h)-1) ;
		lcd_write_bits( p, msk ) ;
    p += DISPLAY_W ;
	}
    
  while( h >= 8 )
	{
		h -= 8 ;
		lcd_write_bits( p, 0xFF ) ;
    p += DISPLAY_W ;
  }
	if ( h > 0 )
	{
  	lcd_write_bits( p, (BITMASK(h)-1) ) ;
	}
#endif
}
#endif
#endif

void lcd_line( coord_t x1, coord_t y1, uint16_t x2, uint16_t y2, uint8_t pat, LcdFlags att )
{
  int dx = x2-x1 ;      /* the horizontal distance of the line */
  int dy = y2-y1 ;      /* the vertical distance of the line */
  int dxabs = abs(dx) ;
  int dyabs = abs(dy) ;
  int sdx = dx > 0 ? 1 : dx < 0 ? -1 : 0 ;
  int sdy = dy > 0 ? 1 : dy < 0 ? -1 : 0 ;
  int x = dyabs>>1 ;
  int y = dxabs>>1 ;
  int px = x1 ;
  int py = y1 ;

	lcd_plot( x2, y2 ) ;

  if (dxabs >= dyabs)
	{
    /* the line is more horizontal than vertical */
    for (int i=0; i<dxabs; i++)
		{
      if ((1<<(px%8)) & pat)
			{
//        lcdDrawPoint(px, py, att);
        lcd_plot(px, py ) ;
      }
      y += dyabs ;
      if (y>=dxabs)
			{
        y -= dxabs ;
        py += sdy ;
      }
      px += sdx ;
    }
  }
  else
	{
    /* the line is more vertical than horizontal */
    for (int i=0; i<dyabs; i++)
		{
      if ((1<<(py%8)) & pat)
			{
//        lcdDrawPoint(px, py, att);
        lcd_plot(px, py ) ;
      }
      x += dxabs;
      if (x >= dyabs)
			{
        x -= dyabs ;
        px += sdx ;
      }
      py += sdy ;
    }
  }
}


#ifdef SIMU
bool lcd_refresh = true;
uint8_t lcd_buf[DISPLAY_W*DISPLAY_H/8];
#endif


#if PCBX9D
#ifdef WIDE_SCREEN
#ifndef REV9E
const uint8_t arrows[] = {
10,64,80,


} ;
#endif
#endif
#endif

#if PCBX9D
uint8_t ImageDisplay = 0 ;
uint8_t ImageX ;
uint8_t ImageY ;
#endif

#if defined(PCBX12D) || defined(PCBX10)
uint8_t ImageDisplay = 0 ;
uint8_t ImageX ;
uint8_t ImageY ;
void lcd_blank()
{
#ifdef WHERE_TRACK
notePosition('1') ;
#endif
//	LCDLastOp = 'I' ;
	lcdDrawSolidFilledRectDMA( 0, 0, 480, 128+64+48, LcdBackground ) ;
//	LCDLastOp = 'J' ;
	lcdDrawSolidFilledRectDMA( 0, 128+64+48, 480, 272-128-64-48, 0 ) ;
}

void lcd_clear()
{
#ifdef WHERE_TRACK
notePosition('2') ;
#endif
//	LCDLastOp = 'K' ;
	lcdDrawSolidFilledRectDMA( 0, 0, 480, 128+64+48, LcdBackground ) ;
//	LCDLastOp = 'L' ;
	lcdDrawSolidFilledRectDMA( 0, 128+64+48, 480, 272-128-64-48, 0 ) ;
//	lcdDrawSolidFilledRectDMA( 0, 0, 480, 128, 0xFFFF ) ;
//	lcdDrawSolidFilledRectDMA( 0, 128, 480-128, 272-128-22, 0 ) ;
//	lcdDrawSolidFilledRectDMA( 480-128, 128+64, 128, 272-128-22-64, 0 ) ;
}
#else
void lcd_clear()
{
  memset( DisplayBuf, 0, sizeof( DisplayBuf) ) ;
#if PCBX9D
#if defined(REVPLUS) || defined(REVNORM)
 #ifndef REV9E
	lcd_img( 212-X9D_OFFSET, 0, arrows, 0, 0 ) ;
	lcd_img( 212-X9D_OFFSET-10, 0, arrows, 1, 0 ) ;
 #endif // nREV9E
#endif	// normal/plus


#endif // PCBX9D
}
#endif // X12D

// LCD i/o pins
// LCD_RES     PC27
// LCD_CS1     PC26
// LCD_E       PC12
// LCD_RnW     PC13
// LCD_A0      PC15
// LCD_D0      PC0
// LCD_D1      PC7
// LCD_D2      PC6
// LCD_D3      PC5
// LCD_D4      PC4
// LCD_D5      PC3
// LCD_D6      PC2
// LCD_D7      PC1

#define LCD_DATA	0x000000FFL
#ifndef REVA
#define LCD_A0    0x00000080L
#else 
#define LCD_A0    0x00008000L
#endif 
#define LCD_RnW   0x00002000L		// bit 13
#define LCD_E     0x00001000L
#define LCD_CS1   0x04000000L		// bit 26
#define LCD_RES   0x08000000L


uint8_t LcdLock ;
uint16_t LcdInputs ;

#ifdef PCBSKY
void lock_lcd()
{
	uint32_t x ;
	uint32_t y ;
	y = PIOC->PIO_PDSR ;
	x = y << 1 ; // 6 LEFT, 5 RIGHT, 4 DOWN, 3 UP
	x &= 0x00FF ;
#ifndef REVA
	if ( y & 0x01000000 )
#else 
	if ( PIOA->PIO_PDSR & 0x80000000 )
#endif
	{
		x |= 0x0400 ;		// EXIT
	}
#ifndef REVA
	if ( PIOB->PIO_PDSR & 0x000000020 )
#else 
	if ( PIOB->PIO_PDSR & 0x000000040 )
#endif
	{
		x |= 0x0200 ;		// MENU
	}
	LcdInputs = x ;
	LcdLock = 1 ;
}
#endif

#ifndef PCBDUE
#ifdef PCBSKY

//#ifndef REVA
uint8_t ErcLcd = 0 ;
//#endif // REVB

const static uint8_t Lcdinit[] =
{
	0xe2,		// Reset
	0xae,		// Display off
	0xa1,		// Reverse segment drive
	0xA6,		// Display normal (not inverse)
	0xA4,		// Display normal (not all on)
	0xA2,		// Bias low (A3 for high)
	0xC0,		// Scan Com0->Com63
	0x2F,		// Internal power mode
	0x25,		// Internal Vreg ratio high(ish)
	0x81,	0x22,		// Contrast
#ifndef REVX
	0xAF		// Display on
#endif
} ;	

const static uint8_t Lcd_ERC12864_2[] =
{
   0xe2, //Initialize the internal functions
   0xae, //DON = 0: display OFF
   0xa1, //ADC = 1: reverse direction(SEG132->SEG1)
   0xA6, //REV = 0: non-reverse display
   0xA4, //EON = 0: normal display. non-entire
   0xA3, // Select LCD bias=0
   0xC0, //SHL = 0: normal direction (COM1->COM64)
   0x2F, //Control power circuit operation VC=VR=VF=1
   0x27, //Select int resistance ratio R2 R1 R0 =5
   0x81, //Set reference voltage Mode
   0x2D, // 24 SV5 SV4 SV3 SV2 SV1 SV0 = 0x18
   0xAF  //DON = 1: display ON
} ;



void lcdInit()
{
	register Pio *pioptr ;
	uint32_t i ;
  // /home/thus/txt/datasheets/lcd/KS0713.pdf
  // ~/txt/flieger/ST7565RV17.pdf  from http://www.glyn.de/content.asp?wdid=132&sid=


#ifndef REVA

// read the inputs, and lock the LCD lines
	lock_lcd() ;
	pioptr = PIOC ;

#if (!defined(REVX) && !defined(ARUNI))
	pioptr->PIO_MDER = LCD_RnW ;		// Open drain
#endif // nREVX && nARUNI
	 
#if defined(REVX) || defined(ARUNI)
	pioptr->PIO_PER = PIO_PC27 | PIO_PC12 | 0xFF ;		// Enable bits 27,12,7-0
#else
	pioptr->PIO_PER = PIO_PC27 | PIO_PC26 | PIO_PC13 | PIO_PC12 | 0xFF ;		// Enable bits 27,26,13,12,7-0
#endif // REVX || ARUNI

#ifdef REVX
	pioptr->PIO_CODR = LCD_E ;
	pioptr->PIO_CODR = LCD_RnW | LCD_CS1 ;	// No longer needed, used elsewhere
#else
	pioptr->PIO_CODR = LCD_E | LCD_RnW | LCD_CS1 ;
#endif // REVX
	pioptr->PIO_SODR = LCD_RES ;
#ifdef ARUNI
	pioptr->PIO_OER = PIO_PC27 | PIO_PC12 | 0xFF ;// Enable bits 27,12,7-0 output
#else
	pioptr->PIO_OER = PIO_PC27 | PIO_PC26 | PIO_PC13 | PIO_PC12 | 0xFF ;		// Set bits 27,26,13,12,7-0 output
#endif
	pioptr->PIO_OWER = 0x000000FFL ;		// Allow write to ls 8 bits in ODSR
#else 
	pioptr = PIOC ;
	pioptr->PIO_PER = 0x0C00B0FFL ;		// Enable bits 27,26,15,13,12,7-0
	pioptr->PIO_CODR = LCD_E | LCD_RnW | LCD_A0 ;
	pioptr->PIO_SODR = LCD_RES | LCD_CS1 ;
	pioptr->PIO_OER = 0x0C00B0FFL ;		// Set bits 27,26,15,13,12,7-0 output
	pioptr->PIO_OWER = 0x000000FFL ;		// Allow write to ls 8 bits in ODSR
#endif // REVA
	
	TC0->TC_CHANNEL[0].TC_CCR = 5 ;	// Enable clock and trigger it (may only need trigger)
	pioptr->PIO_CODR = LCD_RES ;		// Reset LCD
#ifndef REVA
	configure_pins( LCD_A0, PIN_ENABLE | PIN_INPUT | PIN_PORTA | PIN_PULLUP ) ;
#endif // REVA
	while ( TC0->TC_CHANNEL[0].TC_CV < HwDelayScale*HW_COUNT_PER_US*10 )		// >10 uS, Value depends on MCK/8
	{
		// Wait
	}
#ifndef REVA
  if ( ( PIOA->PIO_PDSR & LCD_A0 ) == 0 )
	{
		ErcLcd = 1 ;   // ERC12864-2
	}	 
	configure_pins( LCD_A0, PIN_ENABLE | PIN_LOW | PIN_OUTPUT | PIN_PORTA | PIN_NO_PULLUP ) ;
#endif // REVA
	TC0->TC_CHANNEL[0].TC_CCR = 5 ;	// Enable clock and trigger it (may only need trigger)
	pioptr->PIO_SODR = LCD_RES ;		// Remove LCD reset
	while ( TC0->TC_CHANNEL[0].TC_CV < HwDelayScale*HW_COUNT_PER_US*1500 )	// 1500 uS, Value depends on MCK/8
	{
		// Wait
	}
	for ( i = 0 ; i < sizeof(Lcdinit) ; i += 1 )
	{
#ifndef REVA
	  lcdSendCtl( ErcLcd ? Lcd_ERC12864_2[i] : Lcdinit[i] ) ;
#else
	  lcdSendCtl( Lcdinit[i] ) ;
#endif
	}

#ifdef REVX
// 200mS delay (only if not wdt reset)

extern uint16_t ResetReason ;
	if ( ( ( ResetReason & RSTC_SR_RSTTYP ) != (2 << 8) ) && !unexpectedShutdown )	// Not watchdog
	{
		uint32_t j ;
		for ( j = 0 ; j < 100 ; j += 1 )
		{
			TC0->TC_CHANNEL[0].TC_CCR = 5 ;	// Enable clock and trigger it (may only need trigger)
			while ( TC0->TC_CHANNEL[0].TC_CV < HwDelayScale*HW_COUNT_PER_US*2000 )		// Value depends on MCK/2 (used 18MHz) give 2mS delay
			{
 			  wdt_reset() ;
				// Wait
			}
		}
	}
  lcdSendCtl(0xAF) ; //DON = 1: display ON
#endif // REVX

#ifndef REVA
#ifdef ARUNI
	pioptr->PIO_ODR = 0x0000003BL ;		// Set bits 0, 1, 3, 4, 5 input
	pioptr->PIO_PUER = 0x0000003BL ;		// Set bits 0, 1, 3, 4, 5 with pullups
#else
	pioptr->PIO_ODR = 0x0000003AL ;		// Set bits 1, 3, 4, 5 input
	pioptr->PIO_PUER = 0x0000003AL ;		// Set bits 1, 3, 4, 5 with pullups
	pioptr->PIO_ODSR = 0 ;							// Drive D0 low
#endif  // ARUNI
#else
	pioptr->PIO_ODR = 0x0000003CL ;		// Set bits 2, 3, 4, 5 input
	pioptr->PIO_PUER = 0x0000003CL ;		// Set bits 2, 3, 4, 5 with pullups
	pioptr->PIO_ODSR = 0 ;							// Drive D0 low
#endif // REVA
	LcdLock = 0 ;
}


void lcd_stop()
{
	TC0->TC_CHANNEL[0].TC_CCR = 0 ;	// Enable clock and trigger it (may only need trigger)
	
}

void lcdSetRefVolt(uint8_t val)
{
#ifndef SIMU
	register Pio *pioptr ;
	pioptr = PIOC ;

// read the inputs, and lock the LCD lines
	lock_lcd() ;

#ifdef ARUNI
	pioptr->PIO_OER = 0x080090FFL ;		// Set bits 27,15,12,7-0 output
#else
	pioptr->PIO_OER = 0x0C00B0FFL ;		// Set bits 27,26,15,13,12,7-0 output
#endif

  lcdSendCtl(0x81);
	if ( val == 0 )
	{
		val = 0x22 ;		
	}
  lcdSendCtl(val);
	
#ifndef REVA
#ifdef ARUNI
	pioptr->PIO_ODR = 0x000000FFL ;		// Set LSB 8bits input
	pioptr->PIO_PUER = 0x000000FFL ;	// Set LSB 8bits with pullups
#else
	pioptr->PIO_ODR = 0x000000FEL ;		// Set bits 1, 3, 4, 5 input
	pioptr->PIO_PUER = 0x000000FEL ;		// Set bits 1, 3, 4, 5 with pullups
	pioptr->PIO_ODSR = 0 ;							// Drive D0 low
#endif
#else
	pioptr->PIO_ODR = 0x000000FEL ;		// Set bits 2, 3, 4, 5 input
	pioptr->PIO_PUER = 0x000000FEL ;		// Set bits 2, 3, 4, 5 with pullups
	pioptr->PIO_ODSR = 0 ;							// Drive D0 low
#endif // REVA
#endif // SIMU
	LcdLock = 0 ;
}

void lcdSetOrientation()
{
	Pio *pioptr ;
	pioptr = PIOC ;
// read the inputs, and lock the LCD lines
	lock_lcd() ;

#ifdef ARUNI
	pioptr->PIO_OER = 0x080090FFL ;		// Set bits 27,15,12,7-0 output
#else
	pioptr->PIO_OER = 0x0C00B0FFL ;		// Set bits 27,26,15,13,12,7-0 output
#endif

	uint8_t c1 = 0xA1 ;
	uint8_t c2 = 0xC0 ;
  if (g_eeGeneral.rotateScreen)
	{
		c1 = 0xA0 ;
		c2 = 0xC8 ;
  }
  lcdSendCtl(c1);
  lcdSendCtl(c2);
	c1 = 0xA6 ;		// Display normal (not inverse)
	if (g_eeGeneral.reverseScreen)
	{
		c1 = 0xA7 ;		// Display inverse
	}	
  lcdSendCtl(c1);
#ifndef REVA
#ifdef ARUNI
	pioptr->PIO_ODR = 0x000000FFL ;		// Set LSB 8bits input
	pioptr->PIO_PUER = 0x000000FFL ;	// Set LSB 8bits with pullups
#else
	pioptr->PIO_ODR = 0x000000FEL ;		// Set bits 1, 3, 4, 5 input
	pioptr->PIO_PUER = 0x000000FEL ;		// Set bits 1, 3, 4, 5 with pullups
	pioptr->PIO_ODSR = 0 ;							// Drive D0 low
#endif
#else
	pioptr->PIO_ODR = 0x000000FEL ;		// Set bits 2, 3, 4, 5 input
	pioptr->PIO_PUER = 0x000000FEL ;		// Set bits 2, 3, 4, 5 with pullups
	pioptr->PIO_ODSR = 0 ;							// Drive D0 low
#endif // REVA
	LcdLock = 0 ;
}


void lcdSendCtl(uint8_t val)
{
	register Pio *pioptr ;

	
#ifndef REVA
	pioptr = PIOC ;
#if (!defined(REVX) && !defined(ARUNI))
	pioptr->PIO_CODR = LCD_CS1 ;		// Select LCD
#endif // nREVX && nARUNI
	PIOA->PIO_CODR = LCD_A0 ;
#if (!defined(REVX) && !defined(ARUNI))
	pioptr->PIO_CODR = LCD_RnW ;		// Write
#endif // nREVX && nARUNI
	pioptr->PIO_ODSR = val ;
#else
	pioptr = PIOC ;
	pioptr->PIO_CODR = LCD_CS1 ;		// Select LCD
	pioptr->PIO_CODR = LCD_A0 ;			// Control
	pioptr->PIO_CODR = LCD_RnW ;		// Write
	pioptr->PIO_ODSR = Lcd_lookup[val] ;
#endif 
	
	pioptr->PIO_SODR = LCD_E ;			// Start E pulse
	// Need a delay here (250nS)
	TC0->TC_CHANNEL[0].TC_CCR = 5 ;	// Enable clock and trigger it (may only need trigger)
	while ( TC0->TC_CHANNEL[0].TC_CV < HwDelayScale*HW_COUNT_PER_US/4 )		// Value depends on MCK/8
	{
		// Wait
	}
	pioptr->PIO_CODR = LCD_E ;			// End E pulse
#ifndef REVA
	PIOA->PIO_SODR = LCD_A0 ;				// Data
#else
	pioptr->PIO_SODR = LCD_A0 ;			// Data
#endif	
}

#ifdef SIMU
void refreshDisplay()
{
  memcpy(lcd_buf, DisplayBuf, sizeof(DisplayBuf));
  lcd_refresh = true;
}
#else
void refreshDisplay()
{
	register Pio *pioptr ;
  register uint8_t *p=DisplayBuf;
	register uint32_t y ;
	register uint32_t x ;
	register uint32_t z ;
	register uint32_t ebit ;
	uint16_t delayCount ;
#ifdef PCBSKY
#ifdef REVX
#ifdef RESET_DISPLAY


#endif
#endif
#endif
	delayCount = HwDelayScale*HW_COUNT_PER_US/4 ;		// Value depends on MCK/8
#ifdef PCBSKY
#ifdef REVX
#ifdef RESET_DISPLAY

	delayCount = HwDelayScale*HW_COUNT_PER_US/2 ;		// Value depends on MCK/8

#endif
#endif
#endif

#ifdef PCBSKY
	if ( ( g_model.com2Function == COM2_FUNC_LCD ) || ( g_model.BTfunction == BT_LCDDUMP ) )
	{
		uint16_t time = get_tmr10ms() ;
  	if((uint16_t)( time-ExtDisplayTime) >= 20) //200mS
		{
			ExtDisplayTime = time ;
  		memcpy(&ExtDisplayBuf[1], DisplayBuf, sizeof(DisplayBuf));
			ExtDisplayBuf[0] = 0xAA ;
			ExtDisplayBuf[sizeof(ExtDisplayBuf)-1] = 0x55 ;
			ExtDisplaySend = 1 ;
		}
	}
#endif	// PCBSKY

#ifdef PCBSKY
#ifdef REVX
#ifdef RESET_DISPLAY

//  uint8_t t = keyState((EnumKeys)(SW_BASE_DIAG+0));
  uint8_t t = getSwitch00(33) ;	// LO
	if ( t )
	{
		lcdInit() ;
		lcdSetRefVolt(g_eeGeneral.contrast);
	}

#endif
#endif
#endif


#ifdef REVA
  register uint8_t *lookup ;
	lookup = (uint8_t *)Lcd_lookup ;
#endif // REVA
	ebit = LCD_E ;

//#ifndef REVA
	pioptr = PIOA ;
	pioptr->PIO_PER = 0x00000080 ;		// Enable bit 7 (LCD-A0)
	pioptr->PIO_OER = 0x00000080 ;		// Set bit 7 output
//#endif // REVB

// read the inputs, and lock the LCD lines
	lock_lcd() ;

	pioptr = PIOC ;
#ifndef REVA
#ifdef ARUNI
	pioptr->PIO_OER = 0x080090FFL ;		// Set bits 27,15,12,7-0 output
#else
	pioptr->PIO_OER = 0x0C0030FFL ;		// Set bits 27,26,13,12,7-0 output
#endif
#else
	pioptr->PIO_OER = 0x0C00B0FFL ;		// Set bits 27,26,15,13,12,7-0 output
#endif // REVA
	
  for( y=0; y < 8; y++)
	{
  	uint8_t column_start_lo = 0x04; // skip first 4 columns for normal ST7565
  	if (g_eeGeneral.rotateScreen)
		{
    	column_start_lo ^= 0x04 ;
		}
  	if (g_eeGeneral.optrexDisplay)
		{
    	column_start_lo ^= 0x04 ;
		}
    lcdSendCtl( column_start_lo ) ;
    lcdSendCtl(0x10); //column addr 0
    lcdSendCtl( y | 0xB0); //page addr y
    
#if (!defined(REVX) && !defined(ARUNI))
		pioptr->PIO_CODR = LCD_CS1 ;		// Select LCD
#endif // nREVX && nARUNI

		PIOA->PIO_SODR = LCD_A0 ;			// Data
#if (!defined(REVX) && !defined(ARUNI))
		pioptr->PIO_CODR = LCD_RnW ;		// Write
#endif // nREVX && nARUNI
		 
#ifndef REVA
		x =	*p ;
#else 
		x =	lookup[*p] ;
#endif // REVA
    for( z=0; z<128; z+=1)
		{

			pioptr->PIO_ODSR = x ;
			pioptr->PIO_SODR = ebit ;			// Start E pulse
			// Need a delay here (250nS)
			TC0->TC_CHANNEL[0].TC_CCR = 5 ;	// Enable clock and trigger it (may only need trigger)
			p += 1 ;
#ifndef REVA
		x =	*p ;
#else 
		x =	lookup[*p] ;
#endif // REVA
			while ( TC0->TC_CHANNEL[0].TC_CV < delayCount )		// Value depends on MCK/8
			{
				// Wait
			}
			pioptr->PIO_CODR = ebit ;			// End E pulse
    }
  }
	pioptr->PIO_ODSR = 0xFF ;					// Drive lines high
#ifdef ARUNI
	pioptr->PIO_PUER = 0x000000FFL ;	// Set LSB 8bits with pullups
	pioptr->PIO_ODR = 0x000000FFL ;		// Set LSB 8bits input
#else
#ifndef REVA
	pioptr->PIO_PUER = 0x000000FEL ;	// Set bits 1, 3, 4, 5 with pullups
	pioptr->PIO_ODR = 0x000000FEL ;		// Set bits 1, 3, 4, 5 input
#else
	pioptr->PIO_PUER = 0x000000FEL ;	// Set bits 2, 3, 4, 5 with pullups
	pioptr->PIO_ODR = 0x000000FEL ;		// Set bits 2, 3, 4, 5 input
#endif // REVA
	pioptr->PIO_ODSR = 0xFE ;					// Drive D0 low
#endif

	LcdLock = 0 ;

}

#endif // SIMU
#endif // PCBSKY
#endif // PCBDUE

#ifdef PCB9XT
void refreshDisplay()
{
	if ( g_model.BTfunction == BT_LCDDUMP )
	{
		uint16_t time = get_tmr10ms() ;
  	if((uint16_t)( time-ExtDisplayTime) >= 20) //200mS
		{
			ExtDisplayTime = time ;
  		memcpy(&ExtDisplayBuf[1], DisplayBuf, sizeof(DisplayBuf));
			ExtDisplayBuf[0] = 0xAA ;
			ExtDisplayBuf[sizeof(ExtDisplayBuf)-1] = 0x55 ;
			ExtDisplaySend = 1 ;
		}
	}
	
	if ( ( m64ReceiveStatus() & 1 ) == 0 )
	{
		lcd_char_inverse( 0, 0, 6, 1 ) ;
	}
	displayToM64() ;	
}

void lcdSetRefVolt(uint8_t val)
{
	M64Contrast = val ;
	M64SetContrast = 1 ;
}

void lcdSetOrientation()
{
	M64SetContrast = 1 ;
}

void lcdInit()
{
}

void backlight_on()
{
	uint32_t r ;
	uint32_t g ;
	uint32_t b ;
	
	r = g_eeGeneral.bright ;
	if ( r < 101 )
	{
		r = 100 - r ;
	}
	g = g_eeGeneral.bright_white ;
	if ( g < 101 )
	{
		g = 100 - g ;
	}
	b = g_eeGeneral.bright_blue ;
	if ( b < 101 )
	{
		b = 100 - b ;
	}
	BlSetAllColours( r, g, b ) ;
}

void backlight_off()
{
	BlSetColour( 0, 3 ) ;
}
#endif // PCB9XT
